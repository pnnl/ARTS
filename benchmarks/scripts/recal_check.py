...
misconfigured, judged ONLY by the calibration-baseline protocol ocr_val_wb.

User rule (2026-07-06): an app whose ocr_val_wb 1n cell TIMEOUTs (or whose
ocr_val_wb 1n median falls far outside the ~3 s target: <1.5 s or >6 s) was
sized wrong from the start -> retune to a natural number, invalidate its cells,
re-measure.  BUT if ocr_val_wb is fine (~3 s) and only a Home variant or a
...
def main():
    ...
    base = {}   # bench -> list of ocr_val_wb 1n e2e seconds (OK cells)
    to = {}     # bench -> saw a ocr_val_wb 1n TIMEOUT/FAIL
    other_to = {}  # bench -> runtimes (non-ocr_val_wb) that TIMEOUT at 1n
    ...
    for bench in sorted(set(base) | set(to)):
        if to.get(bench):
            flags.append((bench, "TIMEOUT", "ocr_val_wb 1n TIMEOUT/FAIL -> retune"))
            continue
        es = base.get(bench, [])
        if not es:
            continue
        med = st.median(es)
        if med > HI:
            flags.append((bench, f"{med:.1f}s", f"ocr_val_wb 1n median > {HI}s -> retune"))
        elif med < LO:
            flags.append((bench, f"{med:.1f}s", f"ocr_val_wb 1n median < {LO}s -> (fixed-size? else retune)"))

    lines = ["# recal flags (baseline = ocr_val_wb 1n 48w)"]
    if not flags:
        lines.append("NONE -- all ocr_val_wb 1n medians within [1.5, 6] s; no parameter retune.")
