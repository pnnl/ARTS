...
def cause(metrics, bench, proto="mrnew_lazy", geo="2n_sc"):
...
def main():
    ...
    L = []
    L.append("# RR-divergence survey (all apps x {mrnew_lazy, lock_lazy} x {1n_sc,2n_sc,4n_sc})\n")
    ...
    L.append("\n## Per-app verdict (mrnew_lazy | lock_lazy)\n")
    L.append("| app | expected(scale_skip) | mrnew_lazy e2e 1n/2n/4n | r | verdict | lock_lazy e2e 1n/2n/4n | r | verdict |")
    L.append("|---|---|---|---|---|---|---|---|")
    verdicts = {}
    for bench in order:
    ...
    L.append("## Cause-family (counter spot-check @2n_sc, mrnew_lazy)\n")
