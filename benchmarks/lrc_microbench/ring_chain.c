...
void hop_edt(uint32_t paramc, const uint64_t *paramv, uint32_t depc,
             arts_edt_dep_t *depv) {
  (void)paramc;
  (void)depc;
...
void main_edt(uint32_t paramc, const uint64_t *paramv, uint32_t depc,
  ...
  for (int i = 0; i < g_iters; i++) {
    void *addr;
    arts_guid_t db =
        arts_db_create(&addr, 4096, ARTS_DB, ARTS_DB_PROP_NONE, NULL);
    memset(addr, 0, 4096);
...
int main(int argc, char **argv) {
  if (argc > 1) {
    g_iters = (int)strtol(argv[1], NULL, 10);
  }
  arts_rt(argc, argv);
  return 0;
}
