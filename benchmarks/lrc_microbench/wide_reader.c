...
void reader_edt(uint32_t paramc, const uint64_t *paramv, uint32_t depc,
  ...
  uint64_t sum = 0;
  for (uint64_t i = 0; i < g_db_size; i++) {
    sum += p[i];
  }
...
void main_edt(uint32_t paramc, const uint64_t *paramv, uint32_t depc,
  ...
  for (int i = 0; i < g_iters; i++) {
    void *addr;
    arts_guid_t db =
        arts_db_create(&addr, g_db_size, ARTS_DB, ARTS_DB_PROP_NONE, NULL);
...
int main(int argc, char **argv) {
  if (argc > 1) {
    g_k = (int)strtol(argv[1], NULL, 10);
  }
  if (argc > 2) {
    g_db_size = (uint64_t)strtoll(argv[2], NULL, 10);
  }
  if (argc > 3) {
    g_iters = (int)strtol(argv[3], NULL, 10);
  }
  arts_rt(argc, argv);
  return 0;
}
