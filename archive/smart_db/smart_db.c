...
arts_smart_db_t *arts_smart_db_create_with_guid(arts_guid_t guid, uint64_t size,
  ...
  void *data =
      arts_db_create_with_guid(guid, size, ARTS_DB_DEFAULT, NULL, NULL);
  if (!data) {
    arts_free(smart_db);
    return NULL;
  }
...
void arts_smart_db_set_data(arts_smart_db_t *smart_db, void *data,
  ...
  void *current_data = arts_db_create_with_guid(
      smart_db->db_guid, smart_db->size, ARTS_DB_DEFAULT, NULL, NULL);
  if (!current_data) {
    return;
  }
