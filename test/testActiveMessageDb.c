...
void getter(uint32_t paramc, uint64_t *paramv, uint32_t depc,
            artsEdtDep_t depv[]) {
  unsigned int *buffer;
  artsGuid_t cpyDb = artsDbCreate(
      (void **)&buffer, sizeof(unsigned int) * blockSize, ARTS_DB_READ);
  ...
  artsGuid_t am =
      artsActiveMessageWithDb(setter, paramc, paramv, 1, dbDestGuid);
...
void initPerWorker(unsigned int nodeId, unsigned int workerId, int argc,
  ...
  if (!workerId) {
    ...
    if (!nodeId) {
      ...
      artsDbCreateWithGuidAndData(dbSourceGuid, data,
