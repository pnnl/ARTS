...
void readTest(uint32_t paramc, uint64_t *paramv, uint32_t depc,
              artsEdtDep_t depv[]) {
  //    PRINTF("READ\n");
  unsigned int *array = (unsigned int *)depv[0].ptr;
  for (unsigned int i = 0; i < numWrites; i++) {
...
void writeTest(uint32_t paramc, uint64_t *paramv, uint32_t depc,
               artsEdtDep_t depv[]) {
...
void initPerNode(unsigned int nodeId, int argc, char **argv) {
  ...
  if (!nodeId)
    PRINTF("Reads: %u Writes: %u Dynamic Reads: %u Dynamic Writes: %u Final "
           "Deps: %u\n",
           numReads, numWrites, numDynamicReads, numDynamicWrites,
           numDynamicReads * numWrites + numDynamicWrites * numWrites +
               numReads + numWrites);

  readGuids = (artsGuid_t *)artsMalloc(sizeof(artsGuid_t) * numReads);
  writeGuids = (artsGuid_t *)artsMalloc(sizeof(artsGuid_t) * numWrites);
  ...
  if (!workerId) {
    if (!nodeId) {
      unsigned int *ptr = (unsigned int *)artsDbCreateWithGuid(
          dbGuid, sizeof(unsigned int) * numWrites);
      for (unsigned int i = 0; i < numWrites; i++)
        ptr[i] = 0;

      artsEdtCreateWithGuid(shutdownEdt, shutdownGuid, 0, NULL,
                            numDynamicReads * numWrites +
                                numDynamicWrites * numWrites + numReads +
                                numWrites);
    }
