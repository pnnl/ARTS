...
void creater(uint32_t paramc, uint64_t *paramv, uint32_t depc,
             artsEdtDep_t depv[]) {
  unsigned int *data =
  ...
  if (!workerId) {
    ...
    if (!nodeId) {
      for (unsigned int j = 0; j < deps; j++) {
        artsGetFromDb(edtGuidFixed, dbGuid, j,
                      sizeof(unsigned int) * (nodeId * blockSize + j * stride),
                      sizeof(unsigned int) * stride);
      }
    } else {
      artsGuid_t edtGuid = artsEdtCreate(getter, nodeId, 0, NULL, deps);
      for (unsigned int j = 0; j < deps; j++) {
        artsGetFromDb(edtGuid, dbGuid, j,
                      sizeof(unsigned int) * (nodeId * blockSize + j * stride),
                      sizeof(unsigned int) * stride);
      }
    }
