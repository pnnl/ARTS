...
void initPerWorker(unsigned int nodeId, unsigned int workerId, int argc,
  ...
  if (!workerId) {
    if (nodeId == 0) {
      // Local to local
      unsigned int *aPtr =
          (unsigned int *)artsDbCreateWithGuid(guid[0], sizeof(unsigned int));
      *aPtr = 1;
      artsDbMove(guid[0], 0);

      // Local to remote
      unsigned int *aPtr2 =
          (unsigned int *)artsDbCreateWithGuid(guid[1], sizeof(unsigned int));
      *aPtr2 = 2;
      artsDbMove(guid[1], 1);
    ...
    if (nodeId == 1) {
      unsigned int *bPtr =
          (unsigned int *)artsDbCreateWithGuid(guid[2], sizeof(unsigned int));
      *bPtr = 3;

      unsigned int *cPtr =
          (unsigned int *)artsDbCreateWithGuid(guid[3], sizeof(unsigned int));
      *cPtr = 4;
    }
  }
  if (!workerId) {
