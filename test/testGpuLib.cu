...
void work(uint32_t paramc, uint64_t *paramv, uint32_t depc,
          artsEdtDep_t depv[]) {
  ...
  // al ,bet -scalars
  CHECKCUBLASERROR(cublasSgemm(handle[artsGetGpuId()], CUBLAS_OP_N, CUBLAS_OP_N,
                               m, n, k, &al, d_a, m, d_b, k, &bet, d_c, m));

  float *finalData;
  artsGuid_t finalGuid =
      artsDbCreate((void **)&finalData, sizeof(float) * m * n, ARTS_DB_READ);
...
extern "C" void initPerWorker(unsigned int nodeId, unsigned int workerId,
  ...
  if (!workerId) {
    ...
    artsGuid_t workGuid = artsEdtCreateGpuLib(work, 0, 1, (uint64_t *)&doneGuid,
                                              0, grid, threads);
  }
}
