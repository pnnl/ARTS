...
void copyBlock(unsigned int x, unsigned int y, unsigned int tileRowSize,
               float *tile, unsigned int rowSize, float *mat, bool toTile) {
...
void initBlockMM(uint32_t paramc, uint64_t *paramv, uint32_t depc,
                 artsEdtDep_t depv[]) {
  ...
  float *aTile =
      (float *)artsDbCreateWithGuid(aGuid, sizeof(float) * TILE * TILE);
  float *bTile =
      (float *)artsDbCreateWithGuid(bGuid, sizeof(float) * TILE * TILE);
...
void initPerNode(unsigned int nodeId, int argc, char **argv) {
  ...
  if (!nodeId) {
    float *aMat = (float *)artsDbCreateWithGuid(aMatGuid, MATSIZE * MATSIZE *
                                                              sizeof(float));
    float *bMat = (float *)artsDbCreateWithGuid(bMatGuid, MATSIZE * MATSIZE *
                                                              sizeof(float));
    float *cMat = (float *)artsDbCreateWithGuid(cMatGuid, MATSIZE * MATSIZE *
                                                              sizeof(float));
...
void initPerWorker(unsigned int nodeId, unsigned int workerId, int argc,
  ...
  if (!nodeId && !workerId) {
    artsGuid_t doneGuid =
        artsEdtCreate(finishBlockMM, 0, 0, NULL, 1 + numBlocks * numBlocks);
...
//
// void create(uint32_t paramc, uint64_t * paramv, uint32_t depc, artsEdtDep_t
// depv[])
//{
//     unsigned int i = paramv[0];
...
// }
//
// void work(uint32_t paramc, uint64_t * paramv, uint32_t depc, artsEdtDep_t
// depv[])
//{
...
// }
//
// void stage(uint32_t paramc, uint64_t * paramv, uint32_t depc, artsEdtDep_t
// depv[])
//{
//     artsGuid_t toSignal = paramv[0];
//     unsigned int i      = paramv[1];
...
// }
//
// void finish(uint32_t paramc, uint64_t * paramv, uint32_t depc, artsEdtDep_t
// depv[])
//{
//     PRINTF("DONE\n");
//     artsShutdown();
