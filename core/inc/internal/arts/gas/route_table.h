...
struct arts_route_invalidate {
  int size;
  int used;
  struct arts_route_invalidate *next;
  unsigned int data[];
};
...
typedef struct __attribute__((aligned)) {
  ...
  struct arts_out_of_order_list ooList;
} arts_route_item_t;

typedef struct arts_route_table arts_route_table_t;
...
struct arts_route_table {
  arts_route_item_t *data;
  unsigned int size;
  unsigned int shift;
  struct arts_route_table *next;
