...
#ifdef USE_CXL
  if (mode == ARTS_DB_CXL && (!ptr)) {
    ptr = ((struct artsDb *)artsGetCXLPtr(dataGuid)) + 1;
  }
#endif /* USE_CXL */
...
#ifdef USE_CXL
  if (mode == ARTS_DB_CXL) {
    struct artsDb *db = artsGetCXLPtr(dataGuid);
    db_ptr = (void *)(db + 1);
    assert(db_ptr && "artsSignalEdt ptr not nil");
    // internalSignalEdt(edtGuid, slot, acqGuid, mode, db_ptr, 0);
  }
#endif
  internalSignalEdt(edtGuid, slot, acqGuid, mode, db_ptr, 0);
// #endif /* USE_CXL */
...
#ifdef USE_CXL
volatile uint64_t createdEdts = 0;
volatile uint64_t destroyedEdts = 0;
#endif /* USE_CXL */
