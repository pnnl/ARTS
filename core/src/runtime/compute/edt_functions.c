...
__thread struct arts_edt *current_edt = NULL;
...
void arts_set_thread_local_edt_info(struct arts_edt *edt) {
...
bool arts_edt_create_internal(struct arts_edt *edt, arts_type_t mode,
                           ...
                           uint64_t arts_id) {
  if (!edt) {
    edt = (struct arts_edt *)ARTS_CALLOC_ALIGN_WITH_TYPE(1, edt_space, 16,
...
arts_guid_t arts_edt_create_dep(arts_edt_t func_ptr, unsigned int route,
  ...
  unsigned int edt_space =
      sizeof(struct arts_edt) + (paramc * sizeof(uint64_t)) + dep_space;
...
void arts_edt_free(struct arts_edt *edt) {
  arts_thread_info.edt_free = 1;
  arts_free(edt);
  arts_thread_info.edt_free = 0;
}

void arts_edt_delete(struct arts_edt *edt) {
  if (!edt) {
...
void arts_edt_destroy(arts_guid_t guid) {
  struct arts_edt *edt = (struct arts_edt *)arts_route_table_lookup_item(guid);
  if (!edt) {
...
void *arts_get_depv(void *edt_ptr) {
  struct arts_edt *edt = (struct arts_edt *)edt_ptr;
...
void internal_signal_edt(arts_guid_t edt_packet, uint32_t slot, arts_guid_t data_guid,
  ...
  if (current_edt && current_edt->invalidateCount > 0) {
    ...
    if (rank == arts_global_rank_id) {
      struct arts_edt *edt =
          (struct arts_edt *)arts_route_table_lookup_item(edt_packet);
      if (edt) {
...
void arts_signal_edt_value(arts_guid_t edt_guid, uint32_t slot, uint64_t value) {
  internal_signal_edt(edt_guid, slot, value, ARTS_SINGLE_VALUE, NULL, 0);
}
