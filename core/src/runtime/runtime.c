...
struct arts_runtime_shared arts_node_info;
__thread struct arts_runtime_private arts_thread_info;

typedef bool (*scheduler_t)(void);
#ifdef USE_GPU
scheduler_t scheduler_loop[] = {(scheduler_t)arts_default_scheduler_loop,
...
void arts_runtime_node_init(unsigned int worker_threads,
                         ...
                         struct arts_config *config) {
  arts_thread_set_os_thread_count(config->os_thread_count);
  arts_node_info.scheduler = scheduler_loop[config->scheduler];
  arts_node_info.deque = (struct arts_deque **)arts_malloc(
      sizeof(struct arts_deque *) * total_threads);
  arts_node_info.receiver_deque =
      receiver_threads ? (struct arts_deque **)arts_malloc(
                            sizeof(struct arts_deque *) * receiver_threads)
                      : NULL;
  arts_node_info.gpu_deque = (struct arts_deque **)arts_malloc(
      sizeof(struct arts_deque *) * total_threads);
  arts_node_info.route_table =
      (arts_route_table_t **)arts_calloc(total_threads, sizeof(arts_route_table_t *));
  arts_node_info.gpu_route_table =
      config->gpu ? (arts_route_table_t **)arts_calloc(config->gpu,
                                                    sizeof(arts_route_table_t *))
                  : NULL;
  arts_node_info.remote_route_table =
      arts_new_route_table(config->route_table_entries, config->route_table_size);
  ...
  arts_node_info.atomic_waits = (struct atomic_create_barrier_info **)arts_calloc(
      total_threads, sizeof(struct atomic_create_barrier_info *));
  ...
  for (unsigned int t = 0; t < total_threads; t++) {
    arts_node_info.saved_counters[t] =
        (arts_counter_t *)arts_calloc(NUM_COUNTER_TYPES, sizeof(arts_counter_t));
  }
  ...
  arts_node_info.capture_arrays =
      (arts_array_list_t ***)arts_calloc(total_threads, sizeof(arts_array_list_t **));
  for (unsigned int t = 0; t < total_threads; t++) {
...
void arts_runtime_private_init(struct thread_mask *unit,
                            struct arts_config *config) {
  ...
  if (unit->worker) {
    arts_node_info.route_table[unit->id] =
        arts_new_route_table(config->route_table_entries, config->route_table_size);
#ifdef USE_GPU
...
void arts_handle_remote_stolen_edt(struct arts_edt *edt) {
...
void arts_handle_ready_edt(struct arts_edt *edt) {
...
void arts_run_edt(struct arts_edt *edt) {
  uint32_t depc = edt->depc;
  arts_edt_dep_t *depv = (arts_edt_dep_t *)(((uint64_t *)(edt + 1)) + edt->paramc);
...
inline struct arts_edt *arts_runtime_steal_from_network() {
  ...
  if (arts_global_rank_count > 1) {
    ...
    for (unsigned int i = 0; i < arts_node_info.receiver_thread_count; i++) {
      ...
      if ((edt = (struct arts_edt *)arts_deque_pop_back(
...
inline struct arts_edt *arts_runtime_steal_from_worker() {
  struct arts_edt *edt = NULL;
  if (arts_node_info.total_thread_count > 1) {
    ...
    edt = (struct arts_edt *)arts_deque_pop_back(arts_node_info.deque[steal_loc]);
  }
  return edt;
}

bool arts_network_first_scheduler_loop() {
  struct arts_edt *edt_found;
  if (!(edt_found = arts_runtime_steal_from_network())) {
    if (!(edt_found = (struct arts_edt *)arts_deque_pop_front(
              arts_thread_info.my_node_deque))) {
      if (!(edt_found =
                (struct arts_edt *)arts_deque_pop_front(arts_thread_info.my_deque))) {
...
bool arts_network_before_steal_scheduler_loop() {
  struct arts_edt *edt_found;
  if (!(edt_found =
            (struct arts_edt *)arts_deque_pop_front(arts_thread_info.my_node_deque))) {
    if (!(edt_found =
              (struct arts_edt *)arts_deque_pop_front(arts_thread_info.my_deque))) {
      if (!(edt_found = arts_runtime_steal_from_network())) {
        edt_found = arts_runtime_steal_from_worker();
...
struct arts_edt *arts_find_edt() {
  struct arts_edt *edt_found = NULL;
  if (!(edt_found =
            (struct arts_edt *)arts_deque_pop_front(arts_thread_info.my_deque))) {
    if (!edt_found) {
