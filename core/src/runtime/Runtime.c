...
#ifdef USE_CXL
  artsNodeInfo.cxlDeque = artsCXLDequeInit();
  pthread_mutex_init(&(artsNodeInfo.cxlLocalLock), NULL);
  // Get db arena range
  assert(artsCXLDequeGetDbArenaRange(artsNodeInfo.cxlDeque,
                                     &(artsNodeInfo.cxlDbArenaStartPtr),
                                     &(artsNodeInfo.cxlDbArenaEndPtr)) &&
         "Db Arena pointers valid");
#endif /* USE_CXL */
...
#ifdef USE_CXL
  artsCXLDequeFree(artsNodeInfo.cxlDeque);
#endif /* USE_CXL */
#ifdef USE_GPU
  if (artsNodeInfo.gpu)
    artsCleanupGpus();
...
#ifdef USE_CXL
  if ((artsNodeInfo.scheduler == artsCXLSchedulerLoop) &&
      (artsDequeFull(artsThreadInfo.myDeque))) {
    // Push to CXL
    while (!artsCXLDequePush(artsNodeInfo.cxlDeque,
                             &(artsNodeInfo.cxlLocalLock), edt->header.size,
                             edt))
      ;
    return;
  }
...
#ifdef USE_CXL
bool artsCXLSchedulerLoop() {
  struct artsEdt *edtFound = NULL;
  edtFound = artsDequePopFront(artsThreadInfo.myDeque);
  if (!(edtFound)) {
    if (!(edtFound = artsRuntimeStealFromWorker())) {
      size_t edtSize;
      uint64_t node;
      if (artsCXLDequePop(artsNodeInfo.cxlDeque, &(artsNodeInfo.cxlLocalLock),
                          (void **)&edtFound)) {
