...
static inline void arts_fill_packet_header(struct arts_remote_packet *header,
                                        uint64_t size,
...
static void arts_clear_exclusive_request(struct arts_db *db, int rank,
...
static void send_remote_add_dependence_packet(unsigned int message_type,
  ...
  struct arts_remote_add_dependence_packet packet;
  packet.source = source;
  packet.destination = destination;
  packet.slot = slot;
  packet.acquire_mode = acquire_mode;
...
void arts_remote_add_dependence_with_hints(arts_guid_t source, arts_guid_t destination,
                                      uint32_t slot, unsigned int rank,
                                      arts_type_t acquire_mode) {
  ARTS_DEBUG("Remote Add dependence (acquire=%u) sent %d", acquire_mode, rank);
  send_remote_add_dependence_packet(ARTS_REMOTE_ADD_DEPENDENCE_MSG, source,
                                destination, slot, rank, acquire_mode);
}
...
void arts_remote_add_dependence_to_persistent_event_with_byte_offset(
  ...
  ARTS_DEBUG("Remote Add dep to persistent event with byte offset "
             ...
             acquire_mode, byte_offset, size, rank);
  struct arts_remote_add_dependence_with_byte_offset_packet packet;
  packet.source = source;
  packet.destination = destination;
  packet.slot = slot;
  packet.acquire_mode = acquire_mode;
...
void arts_remote_update_route_table(arts_guid_t guid, unsigned int rank) {
  ...
  if (owner == arts_global_rank_id) {
    struct arts_db_frontier_iterator *iter =
    ...
    struct arts_remote_guid_only_packet packet;
...
void arts_remote_handle_update_db_guid(void *ptr) {
  struct arts_remote_guid_only_packet *packet =
      (struct arts_remote_guid_only_packet *)ptr;
...
void arts_remote_db_destroy(arts_guid_t guid, unsigned int origin_rank, bool clean) {
  ...
  //    {
  //        struct arts_route_invalidate * table =
  //        arts_route_table_get_rank_duplicates(guid); struct arts_route_invalidate *
  //        next = table; struct arts_route_invalidate * current;
  //
...
void arts_remote_update_db(arts_guid_t guid, bool send_db) {
  ...
  if (rank != arts_global_rank_id) {
    struct arts_remote_guid_only_packet packet;
    packet.guid = guid;
    struct arts_db *db = NULL;
    if (send_db && (db = (struct arts_db *)arts_route_table_lookup_item(guid))) {
      if ((db->header.size - sizeof(struct arts_db)) == 176128) {
      ...
      uint64_t size = sizeof(struct arts_remote_guid_only_packet) + db->header.size;
      ...
      arts_fill_packet_header(&packet.header,
                           sizeof(struct arts_remote_guid_only_packet),
...
void arts_remote_handle_update_db(void *ptr) {
  struct arts_remote_guid_only_packet *packet =
      (struct arts_remote_guid_only_packet *)ptr;
  struct arts_db *packet_db = (struct arts_db *)(packet + 1);
  unsigned int rank = arts_guid_get_rank(packet->guid);
  if (rank == arts_global_rank_id) {
    struct arts_db **data_ptr;
    bool write = packet->header.size > sizeof(struct arts_remote_guid_only_packet);
    item_state_t state = arts_route_table_lookup_item_with_state(
        packet->guid, (void ***)&data_ptr, ALLOCATED_KEY, write);
    struct arts_db *db = (data_ptr) ? *data_ptr : NULL;
    if (write && db &&
        (db->header.size - sizeof(struct arts_db)) == 176128) {
    ...
    if (write) {
      void *ptr = (void *)(db + 1);
      memcpy(ptr, packet_db + 1, db->header.size - sizeof(struct arts_db));
      arts_route_table_set_rank(packet->guid, arts_global_rank_id);
...
void arts_remote_memory_move(unsigned int route, arts_guid_t guid, void *ptr,
  ...
  packet.guid = guid;
  arts_remote_send_request_payload_async_free(route, (char *)&packet, sizeof(packet),
...
void arts_remote_memory_move_no_free(unsigned int route, arts_guid_t guid, void *ptr,
  ...
  arts_fill_packet_header(&packet.header, sizeof(packet) + mem_size, message_type);
  packet.guid = guid;
  arts_remote_send_request_payload_async(route, (char *)&packet, sizeof(packet),
                                    (char *)ptr, mem_size);
}
...
void arts_remote_handle_edt_move(void *ptr) {
  struct arts_remote_guid_only_packet *packet =
      (struct arts_remote_guid_only_packet *)ptr;
  uint64_t size =
      packet->header.size - sizeof(struct arts_remote_guid_only_packet);
  struct arts_edt *edt =
      (struct arts_edt *)ARTS_MALLOC_ALIGN_WITH_TYPE(size, 16, ARTS_EDT_MEMORY_SIZE);
...
void arts_remote_handle_db_move(void *ptr) {
  ...
  struct arts_db *db_header = (struct arts_db *)(packet + 1);
  uint64_t db_size = db_header->header.size;

  struct arts_header *mem_packet = (struct arts_header *)ARTS_MALLOC_ALIGN_WITH_TYPE(
  ...
  if (db_header->db_list) {
    struct arts_db *new_db = (struct arts_db *)mem_packet;
...
static void send_remote_edt_signal_packet(arts_guid_t edt, arts_guid_t db,
                                      ...
                                      arts_type_t acquire_mode) {
  struct arts_remote_edt_signal_packet packet;
...
void arts_remote_signal_edt(arts_guid_t edt, arts_guid_t db, uint32_t slot,
  ...
  send_remote_edt_signal_packet(edt, db, slot, mode, ARTS_NULL);
}

void arts_remote_signal_edt_with_hints(arts_guid_t edt, arts_guid_t db, uint32_t slot,
                                  arts_type_t mode, arts_type_t acquire_mode) {
  ARTS_INFO("Remote Signal from DB[Guid:%lu] to EDT[Guid:%lu, Slot:%d, "
            "Rank: %d, AcquireMode:%u]",
            db, edt, slot, arts_guid_get_rank(edt), acquire_mode);
  send_remote_edt_signal_packet(edt, db, slot, mode, acquire_mode);
}

void arts_remote_event_satisfy_slot(arts_guid_t event_guid, arts_guid_t data_guid,
                                uint32_t slot) {
  struct arts_remote_event_satisfy_slot_packet packet;
  packet.event = event_guid;
  packet.db = data_guid;
  packet.slot = slot;
  arts_fill_packet_header(&packet.header, sizeof(packet),
                       ARTS_REMOTE_EVENT_SATISFY_SLOT_MSG);
  arts_remote_send_request_async(arts_guid_get_rank(event_guid), (char *)&packet,
                             sizeof(packet));
}

void arts_remote_persistent_event_satisfy_slot(arts_guid_t event_guid, uint32_t action,
                                          bool lock) {
  struct arts_remote_persistent_event_satisfy_slot_packet packet;
  packet.event = event_guid;
...
static void send_remote_db_add_dependence_packet(arts_guid_t db_src,
                                            ...
                                            uint32_t edt_slot,
                                            arts_type_t acquire_mode) {
  struct arts_remote_db_add_dependence_packet packet;
  ...
  arts_remote_send_request_async(arts_guid_get_rank(db_src), (char *)&packet,
                             sizeof(packet));
}
...
void arts_remote_db_add_dependence_with_hints(arts_guid_t db_src, arts_guid_t edt_dest,
                                        uint32_t edt_slot,
                                        arts_type_t acquire_mode) {
  send_remote_db_add_dependence_packet(db_src, edt_dest, edt_slot, acquire_mode);
}
...
void arts_remote_db_add_dependence_with_byte_offset(arts_guid_t db_src,
                                             ...
                                             uint64_t size) {
  struct arts_remote_db_add_dependence_with_byte_offset_packet packet;
  packet.db_src = db_src;
  packet.edt_dest = edt_dest;
  packet.edt_slot = edt_slot;
  packet.acquire_mode = acquire_mode;
...
void arts_remote_handle_db_add_dependence_with_byte_offset(void *ptr) {
  struct arts_remote_db_add_dependence_with_byte_offset_packet *packet =
      (struct arts_remote_db_add_dependence_with_byte_offset_packet *)ptr;

  /// Look up the local DB
  struct arts_db *db_res =
      (struct arts_db *)arts_route_table_lookup_item(packet->db_src);
  if (db_res != NULL) {
    ...
    arts_add_dependence_to_persistent_event_with_byte_offset(
        db_res->event_guid, packet->edt_dest, packet->edt_slot, packet->acquire_mode,
...
void arts_remote_db_increment_latch(arts_guid_t db) {
  ...
  arts_remote_send_request_async(arts_guid_get_rank(db), (char *)&packet,
                             sizeof(packet));
}
...
void arts_db_request_callback(struct arts_edt *edt, unsigned int slot,
                           struct arts_db *db_res) {
...
bool arts_remote_db_request(arts_guid_t data_guid, int rank, struct arts_edt *edt,
                         int pos, arts_type_t mode, bool agg_request,
                         arts_type_t acquire_mode) {
  if (arts_route_table_add_sent(data_guid, edt, pos, agg_request)) {
    struct arts_remote_db_request_packet packet;
...
void arts_remote_db_send_now(int rank, struct arts_db *db) {
  struct arts_remote_db_send_packet packet;
  uint64_t size = sizeof(struct arts_remote_db_send_packet) + db->header.size;
  arts_fill_packet_header(&packet.header, size, ARTS_REMOTE_DB_SEND_MSG);
  arts_remote_send_request_payload_async(rank, (char *)&packet, sizeof(packet),
                                    (char *)db, db->header.size);
}

void arts_remote_db_send_check(int rank, struct arts_db *db, arts_type_t mode) {
  if (!arts_is_guid_local(db->guid)) {
...
void arts_remote_db_send(struct arts_remote_db_request_packet *pack) {
  ...
  ARTS_INFO("Remote DB Send [Guid:%lu] [Rank: %d] [Mode:%d] [AcquireMode:%d]",
            pack->db_guid, pack->header.rank, pack->mode, pack->acquire_mode);
  if (redirected != arts_global_rank_id && redirected != -1) {
    arts_remote_send_request_async(redirected, (char *)pack, pack->header.size);
  } else {
    struct arts_db *db = (struct arts_db *)arts_route_table_lookup_item(pack->db_guid);
    if (db == NULL) {
      arts_out_of_order_handle_remote_db_send(pack->header.rank, pack->db_guid,
                                       pack->mode);
    } else if (!arts_is_guid_local(db->guid) &&
               pack->header.rank == arts_global_rank_id) {
      // This is when the memory model sends a CDAG write after CDAG write to
      // the same node The arts_is_guid_local should be an extra check, maybe not
      // required
      arts_route_table_fire_oo(pack->db_guid, arts_out_of_order_handler);
    } else {
      arts_remote_db_send_check(pack->header.rank, db, pack->mode);
    }
  }
}

void arts_remote_handle_db_received(struct arts_remote_db_send_packet *packet) {
  struct arts_db *packet_db = (struct arts_db *)(packet + 1);
  ARTS_DEBUG("Handle DB Received [Guid:%lu] on rank %u", packet_db->guid,
             arts_global_rank_id);
  struct arts_db *db_res = NULL;
  struct arts_db **data_ptr = NULL;
  item_state_t state = arts_route_table_lookup_item_with_state(
      packet_db->guid, (void ***)&data_ptr, ALLOCATED_KEY, true);

  struct arts_db *t_ptr = (data_ptr) ? *data_ptr : NULL;
  struct arts_db_list *db_list = NULL;
  if (t_ptr && arts_is_guid_local(packet_db->guid)) {
    db_list = (struct arts_db_list *)t_ptr->db_list;
}
  ARTS_DEBUG("Rec DB State: %u", state);
  switch (state) {
  case REQUESTED_KEY: {
    if (packet_db->header.size == t_ptr->header.size) {
      void *source = (void *)(packet_db + 1);
      void *dest = (void *)(t_ptr + 1);
      memcpy(dest, source, packet_db->header.size - sizeof(struct arts_db));
  ...
  case RESERVED_KEY: {
    db_res = (struct arts_db *)ARTS_MALLOC_ALIGN_WITH_TYPE(packet_db->header.size, 16,
                                                     ARTS_DB_MEMORY_SIZE);
    memcpy(db_res, packet_db, packet_db->header.size);
    if (arts_is_guid_local(packet_db->guid)) {
...
void arts_remote_db_full_request(arts_guid_t data_guid, int rank, arts_guid_t edt_guid,
  ...
  struct arts_remote_db_full_request_packet packet;
  packet.db_guid = data_guid;
...
void arts_remote_db_full_send_now(int rank, struct arts_db *db, arts_guid_t edt_guid,
                             unsigned int slot, arts_type_t mode) {
  struct arts_remote_db_full_send_packet packet;
  packet.edt_guid = edt_guid;
  packet.slot = slot;
  packet.mode = mode;
  uint64_t size = sizeof(struct arts_remote_db_full_send_packet) + db->header.size;
...
void arts_remote_db_full_send_check(int rank, struct arts_db *db, arts_guid_t edt_guid,
                               unsigned int slot, arts_type_t mode) {
  if (!arts_is_guid_local(db->guid)) {
    arts_route_table_return_db(db->guid, false);
...
void arts_remote_db_full_send(struct arts_remote_db_full_request_packet *pack) {
  unsigned int redirected = arts_route_table_lookup_rank(pack->db_guid);
  if (redirected != arts_global_rank_id && redirected != -1) {
    arts_remote_send_request_async(redirected, (char *)pack, pack->header.size);
  } else {
    struct arts_db *db = (struct arts_db *)arts_route_table_lookup_item(pack->db_guid);
    if (db == NULL) {
      arts_out_of_order_handle_remote_db_full_send(pack->db_guid, pack->header.rank,
                                           pack->edt_guid, pack->slot,
                                           pack->mode);
    } else {
      arts_remote_db_full_send_check(pack->header.rank, db, pack->edt_guid,
                                pack->slot, pack->mode);
    }
  }
}

void arts_remote_handle_db_full_recieved(struct arts_remote_db_full_send_packet *packet) {
  bool dec;
  item_state_t state;
  struct arts_db *packet_db = (struct arts_db *)(packet + 1);
  ...
  struct arts_db *db_res = (data_ptr) ? (struct arts_db *)*data_ptr : NULL;
  if (db_res) {
    if (packet_db->header.size == db_res->header.size) {
      struct arts_db_list *db_list = (struct arts_db_list *)db_res->db_list;
      void *source = (void *)(packet_db + 1);
  ...
  struct arts_edt *edt =
      (struct arts_edt *)arts_route_table_lookup_item(packet->edt_guid);
  if (!edt) {
    void **edt_data = NULL;
...
void arts_remote_handle_send_already_local(void *pack) {
  struct arts_remote_db_full_request_packet *packet =
      (struct arts_remote_db_full_request_packet *)pack;
  int rank;
  struct arts_db *db_res =
      (struct arts_db *)arts_route_table_lookup_db(packet->db_guid, &rank, true);
  struct arts_edt *edt =
      (struct arts_edt *)arts_route_table_lookup_item(packet->edt_guid);
  if (!edt) {
    void **edt_data = NULL;
...
void arts_remote_get_from_db(arts_guid_t edt_guid, arts_guid_t db_guid,
  ...
  struct arts_remote_get_put_packet packet;
  packet.edt_guid = edt_guid;
...
void arts_remote_handle_get_from_db(void *pack) {
  struct arts_remote_get_put_packet *packet = (struct arts_remote_get_put_packet *)pack;
...
void arts_remote_put_in_db(void *ptr, arts_guid_t edt_guid, arts_guid_t db_guid,
  ...
  packet.offset = offset;
  packet.size = size;
  uint64_t total_size = sizeof(struct arts_remote_get_put_packet) + size;
  arts_fill_packet_header(&packet.header, total_size, ARTS_REMOTE_PUT_IN_DB_MSG);
  //    arts_remote_send_request_payload_async(rank, (char *)&packet, sizeof(packet),
  //    (char *)ptr, size);
  arts_remote_send_request_payload_async_free(rank, (char *)&packet, sizeof(packet),
                                        (char *)ptr, 0, size, arts_free);
}
...
void arts_remote_signal_edt_with_ptr(arts_guid_t edt_guid, arts_guid_t db_guid,
  ...
  struct arts_remote_signal_edt_with_ptr_packet packet;
  packet.edt_guid = edt_guid;
  packet.db_guid = db_guid;
  packet.size = size;
  packet.slot = slot;
  uint64_t total_size = sizeof(struct arts_remote_signal_edt_with_ptr_packet) + size;
...
void arts_remote_handle_signal_edt_with_ptr(void *pack) {
  struct arts_remote_signal_edt_with_ptr_packet *packet =
      (struct arts_remote_signal_edt_with_ptr_packet *)pack;
...
void arts_remote_metric_update(int rank, int type, int level, uint64_t time_stamp,
  ...
  struct arts_remote_metric_update packet;
  packet.type = type;
...
void arts_remote_send(unsigned int rank, send_handler_t fun_ptr, void *args,
  ...
  struct arts_remote_send packet;
  packet.fun_ptr = fun_ptr;
  int total_size = sizeof(struct arts_remote_send) + size;
...
void arts_remote_handle_send(void *pack) {
  struct arts_remote_send *packet = (struct arts_remote_send *)pack;
...
void arts_remote_epoch_init_send(unsigned int rank, arts_guid_t epoch_guid,
  ...
  struct arts_remote_epoch_init_packet packet;
  packet.epoch_guid = epoch_guid;
  packet.edt_guid = edt_guid;
  packet.slot = slot;
...
void arts_remote_handle_epoch_init_send(void *pack) {
  ...
  struct arts_remote_epoch_init_packet *packet =
      (struct arts_remote_epoch_init_packet *)pack;
...
void arts_remote_epoch_init_pool_send(unsigned int rank, unsigned int pool_size,
  ...
  //    pool_guid);
  struct arts_remote_epoch_init_pool_packet packet;
...
void arts_remote_handle_epoch_init_pool_send(void *pack) {
  ...
  struct arts_remote_epoch_init_pool_packet *packet =
      (struct arts_remote_epoch_init_pool_packet *)pack;
...
void arts_remote_handle_epoch_req(void *pack) {
  struct arts_remote_guid_only_packet *packet =
      (struct arts_remote_guid_only_packet *)pack;
...
void arts_remote_epoch_send(unsigned int rank, arts_guid_t guid,
  ...
  struct arts_remote_epoch_send_packet packet;
...
void arts_remote_handle_epoch_send(void *pack) {
  struct arts_remote_epoch_send_packet *packet =
      (struct arts_remote_epoch_send_packet *)pack;
...
void arts_remote_atomic_add_in_array_db(unsigned int rank, arts_guid_t db_guid,
  ...
  struct arts_remote_atomic_add_in_array_db_packet packet;
  packet.db_guid = db_guid;
  packet.edt_guid = edt_guid;
  packet.epoch_guid = epoch_guid;
  packet.slot = slot;
...
void arts_remote_handle_atomic_add_in_array_db(void *pack) {
  struct arts_remote_atomic_add_in_array_db_packet *packet =
      (struct arts_remote_atomic_add_in_array_db_packet *)pack;
  struct arts_db *db = (struct arts_db *)arts_route_table_lookup_item(packet->db_guid);
...
void arts_remote_atomic_compare_and_swap_in_array_db(
  ...
  struct arts_remote_atomic_compare_and_swap_in_array_db_packet packet;
  packet.db_guid = db_guid;
  packet.edt_guid = edt_guid;
  packet.epoch_guid = epoch_guid;
  packet.slot = slot;
...
void arts_remote_handle_atomic_compare_and_swap_in_array_db(void *pack) {
  struct arts_remote_atomic_compare_and_swap_in_array_db_packet *packet =
      (struct arts_remote_atomic_compare_and_swap_in_array_db_packet *)pack;
  struct arts_db *db = (struct arts_db *)arts_route_table_lookup_item(packet->db_guid);
...
void arts_db_move_request(arts_guid_t db_guid, unsigned int dest_rank) {
  ...
  packet.header.rank = dest_rank;
  arts_remote_send_request_async(arts_guid_get_rank(db_guid), (char *)&packet,
                             sizeof(packet));
}

void arts_db_move_request_handle(void *pack) {
  struct arts_remote_db_request_packet *packet =
      (struct arts_remote_db_request_packet *)pack;
...
void arts_remote_signal_context(unsigned int rank, uint64_t ticket) {
  struct arts_remote_signal_context_packet packet;
...
void arts_remote_handle_signal_context(void *pack) {
  struct arts_remote_signal_context_packet *packet =
      (struct arts_remote_signal_context_packet *)pack;
...
void arts_remote_db_rename(arts_guid_t new_guid, arts_guid_t old_guid) {
  ...
  struct arts_remote_db_rename packet;
  packet.old_guid = old_guid;
  packet.new_guid = new_guid;
  packet.header.size = sizeof(packet);
  packet.header.message_type = ARTS_REMOTE_DB_RENAME_MSG;
  packet.header.rank = dest_rank;
  arts_remote_send_request_async(dest_rank, (char *)&packet, sizeof(packet));
}

void arts_remote_handle_db_rename(void *pack) {
  struct arts_remote_db_rename *packet = (struct arts_remote_db_rename *)pack;
...
void arts_remote_time_sync_request(void) {
  struct arts_remote_time_sync_req_packet packet;
  packet.workerSendTime = arts_get_time_stamp(); // T1
  arts_fill_packet_header(&packet.header, sizeof(packet),
                       ARTS_REMOTE_TIME_SYNC_REQ_MSG);

  // Send to master
  arts_remote_send_request_async(arts_global_master_rank_id, (char *)&packet,
                             sizeof(packet));
...
void arts_remote_handle_time_sync_req(void *pack) {
  struct arts_remote_time_sync_req_packet *req =
      (struct arts_remote_time_sync_req_packet *)pack;
  uint64_t master_recv_time = arts_get_time_stamp(); // T2

  struct arts_remote_time_sync_resp_packet resp;
  ...
  arts_remote_send_request_async(req->header.rank, (char *)&resp, sizeof(resp));
...
void arts_remote_handle_time_sync_resp(void *pack) {
  struct arts_remote_time_sync_resp_packet *resp =
      (struct arts_remote_time_sync_resp_packet *)pack;
  uint64_t worker_recv_time = arts_get_time_stamp(); // T3

  uint64_t T1 = resp->workerSendTime;
  uint64_t T2 = resp->master_recv_time;
  uint64_t T3 = worker_recv_time;
  ...
  ARTS_INFO("Time sync: Worker %u received response, T1=%lu, T2=%lu, T3=%lu, "
            ...
            arts_global_rank_id, T1, T2, T3, rtt, (double)rtt / 1000000.0, offset,
