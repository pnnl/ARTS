...
#ifdef USE_CXL
  if (mode == ARTS_DB_CXL) {
    ptr = artsCXLDequeDbMalloc(artsNodeInfo.cxlDeque,
                               &(artsNodeInfo.cxlLocalLock), size);
  }
#endif /* USE_CXL */
  if (!ptr) {
...
void artsDbCreateInternal(artsGuid_t guid, void *addr, uint64_t size,
  ...
  dbRes->twinSize = 0;
#ifdef USE_CXL
  if (mode != ARTS_DB_PIN && mode != ARTS_DB_CXL) {
  ...
  if (mode != ARTS_DB_CXL) {
    ...
    // dbRes->eventGuid = NULL_GUID;  // CXL doesn't need persistent events
    dbRes->eventGuid = artsPersistentEventCreate(-1, 0, guid);
  }
  #else
  dbRes->eventGuid = artsPersistentEventCreate(artsGuidGetRank(guid), 0, guid);
  #endif
...
#ifdef USE_CXL
  if (dbSize % CACHELINE_SIZE)
    dbSize = ALIGN_UP(dbSize, CACHELINE_SIZE);
#endif /* USE_CXL */
...
#ifdef USE_CXL
    if (mode == ARTS_DB_CXL) {
      guid = artsGetCXLGuid(ptr);
      artsDbCreateInternal(guid, ptr, dbSize - sizeof(struct artsDb), dbSize,
                           mode, 0);
      *addr = (void *)((struct artsDb *)ptr + 1);
      FLUSH_FENCE_PRODUCER(ptr, ALIGN_UP(dbSize, CACHELINE_SIZE));
    } else {
#endif /* USE_CXL */
...
#ifdef USE_CXL
      case ARTS_DB_CXL:
        struct artsDb *dbTemp = artsGetCXLPtr(depv[i].guid);
...
#ifdef USE_CXL
    if (depv[i].mode == ARTS_DB_CXL) {
      struct artsDb *db;
      assert(depv[i].ptr && "prepDbs ptr not nil");
      db = ((struct artsDb *)depv[i].ptr) - 1;
      artsCXLConsumerFlush(artsGetCXLGuid(db));
    }
#endif /* USE_CXL */
#ifdef USE_GPU
...
#ifdef USE_CXL
    else if (depv[i].mode == ARTS_DB_CXL) {
      artsCXLProducerFlush(depv[i].guid);
      // struct artsDb * db = ((struct artsDb *) depv[i].ptr) - 1;
      // DPRINTF("Rank %d, thread: %d: releaseDb, db, db+1, headerSize: %p, %p,
      // %zu\n", artsGlobalRankId, artsThreadInfo.threadId, db, db+1,
      // db->header.size); FLUSH_FENCE_PRODUCER(db, ALIGN_UP(db->header.size,
      // CACHELINE_SIZE));
    }
#endif /* USE_CXL */
    else {
...
#ifdef USE_CXL
void artsCXLProducerFlush(artsGuid_t guid) {
  void *ptr = artsGetCXLPtr(guid);
  // Seg faults here. header size is massive
  // struct artsDb * db = ((struct artsDb *) ptr) - 1;
  struct artsDb *db = ((struct artsDb *)ptr);
  // if (db->header.size > 128) {
  // artsDebugPrintStack();
...
void artsCXLConsumerFlush(artsGuid_t guid) {
  void *ptr = artsGetCXLPtr(guid);
  struct artsDb *db = ((struct artsDb *)ptr);
  // struct artsDb * db = ((struct artsDb *) ptr) - 1;
  // FLUSH_FENCE_CONSUMER(db, CACHELINE_SIZE);
  FLUSH_FENCE_CONSUMER(db, ALIGN_UP(sizeof(struct artsDb), CACHELINE_SIZE));
  if ((db->header.size + sizeof(struct artsDb)) > CACHELINE_SIZE) {
    /*
    if (db->header.size > 128) {
      artsDebugPrintStack();
      printf("db header size: %lu\n", db->header.size);
      fflush(stdout);
      artsDebugGenerateSegFault();
    }
    */
    FLUSH_FENCE_CONSUMER(
        db, ALIGN_UP(db->header.size + sizeof(struct artsDb), CACHELINE_SIZE));
  }
}
#endif /* USE_CXL */
