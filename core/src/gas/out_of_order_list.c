...
bool reader_oo_try_lock(struct arts_out_of_order_list *list) {
  while (1) {
...
inline void reader_oo_lock(struct arts_out_of_order_list *list) {
  while (1) {
...
void reader_oo_unlock(struct arts_out_of_order_list *list) {
  arts_atomic_sub(&list->readerLock, 1U);
}

void writer_oo_lock(struct arts_out_of_order_list *list, unsigned int lock_type) {
...
void writer_oo_unlock(struct arts_out_of_order_list *list) {
  arts_atomic_swap(&list->writerLock, 0U);
}

bool writer_try_oo_lock(struct arts_out_of_order_list *list, unsigned int lock_type) {
...
bool arts_o_ois_fired(struct arts_out_of_order_list *list) { return list->isFired; }

bool arts_out_of_order_list_add_item(struct arts_out_of_order_list *add_to_me, void *item) {
  ...
  volatile struct arts_out_of_order_element *current = &add_to_me->head;
  for (unsigned int i = 0; i < num_elements; i++) {
    if (!current->next) {
      if (i + 1 == num_elements && element_pos == 0) {
        current->next = (struct arts_out_of_order_element *)arts_calloc(
            1, sizeof(struct arts_out_of_order_element));
      } else {
        while (!current->next) {
          ;
}
...
void arts_out_of_order_list_reset(struct arts_out_of_order_list *list) {
...
void delete_oo_elements(struct arts_out_of_order_element *current) {
  struct arts_out_of_order_element *trail = NULL;
  while (current) {
    ...
    trail = current;
    current = (struct arts_out_of_order_element *)current->next;
    arts_free(trail);
  }
}

// Not threadsafe
void arts_out_of_order_list_delete(struct arts_out_of_order_list *list) {
  delete_oo_elements((struct arts_out_of_order_element *)list->head.next);
  list->head.next = NULL;
  list->isFired = false;
  list->count = 0;
}

void arts_out_of_order_list_fire_callback(struct arts_out_of_order_list *fire_me,
  ...
  for (int attempt = 0; attempt < max_retries; attempt++) {
    if (writer_try_oo_lock(fire_me, FIRE_LOCK)) {
      ...
      unsigned int j = 0;
      for (volatile struct arts_out_of_order_element *current = &fire_me->head;
      ...
      fire_me->count = 0;
      struct arts_out_of_order_element *p =
          (struct arts_out_of_order_element *)fire_me->head.next;
