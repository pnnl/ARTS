...
#ifdef USE_CXL
void *artsGetCXLPtr(artsGuid_t guid) {
  artsGuid addressInfo = ((artsGuid)guid);
  uint64_t base_ptr = addressInfo.cxlFields.base_ptr;
  uint64_t masked_ptr = base_ptr & 0x00ffffffffffffff;
  return (void *)(masked_ptr | CXL_BASE_ADDR);
}

artsGuid_t artsGetCXLGuid(void *ptr) {
  artsGuid ret;
  ret.bits = (intptr_t)ptr;
  ret.cxlFields.type = ARTS_DB_CXL;
  return (artsGuid_t)ret.bits;
}
#endif /* USE_CXL */
