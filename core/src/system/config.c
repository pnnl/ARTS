...
char *extract_nodelist_lsf(const char *envr, int stride, unsigned int *cnt) {
  ...
  while (res_string) {
    ...
    if (res_string && strcmp(res_string, last) != 0 && !(ele % stride)) {
      strcpy(&node_list[list_str_length], res_string);
...
struct arts_config_variable *
arts_config_find_variable(struct arts_config_variable **head, const char *string) {
  struct arts_config_variable *found = NULL;
  struct arts_config_variable *last = NULL;
  struct arts_config_variable *next = *head;
  ...
  if (overide) {
    ...
    struct arts_config_variable *new_var = (struct arts_config_variable *)arts_malloc(
        sizeof(struct arts_config_variable) + size);

    new_var->size = size;
    strcpy(new_var->variable, string);
    strcpy(new_var->value, overide);
...
char *arts_config_find_variable_char(struct arts_config_variable *head,
                                 const char *string) {
  struct arts_config_variable *found = NULL;
  char *overide = getenv(string);
...
unsigned int arts_config_get_variable(FILE *config, const char *look_for_me) {
  ...
  char *val;
  int size;
  struct arts_config_variable *c_var;
  struct arts_config_variable *head;
  struct arts_config_variable *next = NULL;
...
struct arts_config_variable *arts_config_get_variables(FILE *config) {
  char *line = NULL;
  size_t len = 0;
  ssize_t read;
  char *var;
  ...
  while ((read = getline(&line, &len, config)) != -1) {
    ...
    if (val != NULL) {
      ...
      c_var = (struct arts_config_variable *)arts_malloc(
          sizeof(struct arts_config_variable) + size);
      c_var->size = size;

      strncpy(c_var->variable, var, 255);
      strcpy(c_var->value, val);
...
char *arts_config_get_slurm_hostname(char *name, char *digit_sample,
  ...
  for (unsigned int i = digit_length; i > 0; i--) {
    out_name[prefix_length + length + i - 1] = ('0') + value % 10;
    value /= 10;
  }
...
void arts_config_create_routing_table(struct arts_config **config, char *node_list) {
  unsigned int node_count;
  struct arts_config_table *table;
  ...
  table = (struct arts_config_table *)arts_calloc(node_count,
                                               sizeof(struct arts_config_table));
...
unsigned int arts_config_get_number_of_threads(char *location) {
  ...
  return arts_config_get_variable(config_file, "threads");
}

struct arts_config *arts_config_load() {
  FILE *config_file = NULL;
  struct arts_config *config;
  struct arts_config_variable *config_variables;
  struct arts_config_variable *found_variable;
  char *found_variable_char;

  char *end = NULL;

  config = (struct arts_config *)arts_calloc(1, sizeof(struct arts_config));
  ...
  if (strncmp(found_variable->value, "slurm", 5) == 0) {
  ...
  if ((found_variable = arts_config_find_variable(&config_variables, "port")) !=
    ...
    if (found_variable->value[0] == '[') {
      ...
      if (sscanf(found_variable->value, "[%u-%u]", &start_port, &end_port) == 2) {
        config->port_range = true;
        config->port_start = start_port;
        config->port_end = end_port;
        config->port = start_port;
      } else {
  ...
  while (config_variables != NULL) {
    struct arts_config_variable *next_var = config_variables->next;
...
void arts_config_destroy(struct arts_config *config) {
