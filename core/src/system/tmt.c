...
static void *arts_alias_thread_loop(void *arg) {
  ...
  memcpy(&arts_thread_info, t_args->tlToCopy, sizeof(struct arts_runtime_private));
...
static inline void arts_create_contexts(struct arts_runtime_private *semi_private,
  ...
  tmask.startUpSem = &local_internal->sem[alias_id % num_at];
  ...
  for (int i = 0; i < end; ++i) {
    ...
    if (sem_wait(&local_internal->sem[alias_id % num_at]) ==
...
void arts_tmt_construct_new_internal_msi(msi_t *root, unsigned int num_at,
                                    struct arts_runtime_private *semi_private) {
...
void arts_tmt_runtime_private_init(struct thread_mask *unit,
                               struct arts_runtime_private *semi_private) {
  local_pool = &arts_t_mt_msi[arts_thread_info.group_id];
