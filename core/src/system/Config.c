...
struct artsConfig *artsConfigLoad() {
  ...
  struct artsConfigVariable *foundVariableChar;
...
void artsConfigPrint(const struct artsConfig *config) {
  if (!config) {
    ARTS_INFO("artsConfigPrint: config is NULL");
    return;
  }

  ARTS_INFO("==== ARTS Configuration ====");

  ARTS_INFO("myRank                : %u", config->myRank);
  ARTS_INFO("masterNode            : %s", config->masterNode ? config->masterNode : "(null)");
  ARTS_INFO("netInterface          : %s", config->netInterface ? config->netInterface : "(null)");
  ARTS_INFO("launcher              : %s", config->launcher ? config->launcher : "(null)");

  ARTS_INFO("ports                 : %u", config->ports);
  ARTS_INFO("osThreadCount         : %u", config->osThreadCount);
  ARTS_INFO("threadCount           : %u", config->threadCount);
  ARTS_INFO("coreCount             : %u", config->coreCount);
  ARTS_INFO("recieverCount         : %u", config->recieverCount);
  ARTS_INFO("senderCount           : %u", config->senderCount);
  ARTS_INFO("nodes                 : %u", config->nodes);
  ARTS_INFO("masterRank            : %u", config->masterRank);
  ARTS_INFO("port                  : %u", config->port);
  ARTS_INFO("killMode              : %u", config->killMode);
  ARTS_INFO("routeTableSize        : %u", config->routeTableSize);
  ARTS_INFO("routeTableEntries     : %u", config->routeTableEntries);
  ARTS_INFO("dequeSize             : %u", config->dequeSize);

  ARTS_INFO("counterFolder         : %s", config->counterFolder ? config->counterFolder : "(null)");
  ARTS_INFO("counterStartPoint     : %u", config->counterStartPoint);
  ARTS_INFO("counterCaptureInterval: %u", config->counterCaptureInterval);
  ARTS_INFO("printNodeStats        : %u", config->printNodeStats);
  ARTS_INFO("scheduler             : %u", config->scheduler);
  ARTS_INFO("shutdownEpoch         : %u", config->shutdownEpoch);

  ARTS_INFO("prefix                : %s", config->prefix ? config->prefix : "(null)");
  ARTS_INFO("suffix                : %s", config->suffix ? config->suffix : "(null)");
  ARTS_INFO("ibNames               : %s", config->ibNames ? "true" : "false");
  ARTS_INFO("masterBoot            : %s", config->masterBoot ? "true" : "false");
  ARTS_INFO("coreDump              : %s", config->coreDump ? "true" : "false");
  ARTS_INFO("pinStride             : %u", config->pinStride);
  ARTS_INFO("printTopology         : %s", config->printTopology ? "true" : "false");
  ARTS_INFO("pinThreads            : %s", config->pinThreads ? "true" : "false");
  ARTS_INFO("shadLoopStride        : %u", config->shadLoopStride);
  ARTS_INFO("stackSize             : %" PRIu64, (uint64_t)config->stackSize);

  ARTS_INFO("tMT                   : %u", config->tMT);
  ARTS_INFO("coresPerNetworkThread : %u", config->coresPerNetworkThread);

  ARTS_INFO("gpu                   : %u", config->gpu);
  ARTS_INFO("gpuLocality           : %u", config->gpuLocality);
  ARTS_INFO("gpuFit                : %u", config->gpuFit);
  ARTS_INFO("gpuLCSync             : %u", config->gpuLCSync);
  ARTS_INFO("gpuMaxEdts            : %u", config->gpuMaxEdts);
  ARTS_INFO("gpuMaxMemory          : %" PRIu64, (uint64_t)config->gpuMaxMemory);
  ARTS_INFO("gpuP2P                : %s", config->gpuP2P ? "true" : "false");
  ARTS_INFO("gpuBuffOn             : %s", config->gpuBuffOn ? "true" : "false");
  ARTS_INFO("gpuRouteTableSize     : %u", config->gpuRouteTableSize);
  ARTS_INFO("gpuRouteTableEntries  : %u", config->gpuRouteTableEntries);
  ARTS_INFO("freeDbAfterGpuRun     : %s", config->freeDbAfterGpuRun ? "true" : "false");
  ARTS_INFO("runGpuGcPreEdt        : %s", config->runGpuGcPreEdt ? "true" : "false");
  ARTS_INFO("runGpuGcIdle          : %s", config->runGpuGcIdle ? "true" : "false");
  ARTS_INFO("deleteZerosGpuGc      : %s", config->deleteZerosGpuGc ? "true" : "false");

  ARTS_INFO("tableLength           : %u", config->tableLength);
  if (config->table && config->tableLength) {
    for (unsigned int i = 0; i < config->tableLength; i++) {
      ARTS_INFO("  table[%u].rank       : %u", i, config->table[i].rank);
      ARTS_INFO("  table[%u].ipAddress  : %s", i,
                config->table[i].ipAddress ? config->table[i].ipAddress : "(null)");
    }
  }

  ARTS_INFO("==== End ARTS Configuration ====");
}
