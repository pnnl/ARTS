...
void set_thread_mask(struct thread_mask *thread_mask, struct unit_mask *unit_mask,
                   struct unit_thread *unit_thread) {
...
void add_a_thread(struct unit_mask *mask, bool work_on, bool network_out_on,
  ...
  if (mask->listHead == NULL) {
    mask->listTail = mask->listHead =
        (struct unit_thread *)arts_malloc(sizeof(struct unit_thread));
    next = mask->listHead;
  } else {
    next = mask->listTail;
    next->next = (struct unit_thread *)arts_malloc(sizeof(struct unit_thread));
    next = next->next;
    mask->listTail = next;
  }
...
void init_cluster_units(hwloc_topology_t topology, hwloc_obj_t obj,
                      ...
                      unsigned int *unit_index, struct unit_mask *units) {
...
struct node_mask *get_node_mask() {
  struct node_mask *node =
      (struct node_mask *)arts_malloc(sizeof(struct node_mask));
  ...
  node->cluster = (struct cluster_mask *)arts_malloc(sizeof(struct cluster_mask) *
                                                   node->num_clusters);
  ...
  for (cluster_index = 0; cluster_index < node->num_clusters; cluster_index++) {
    ...
    node->cluster[cluster_index].core = (struct core_mask *)arts_malloc(
        sizeof(struct core_mask) * node->cluster[cluster_index].num_cores);
    for (core_index = 0; core_index < node->cluster[cluster_index].num_cores;
      ...
      node->cluster[cluster_index].core[core_index].unit =
          (struct unit_mask *)arts_malloc(
              sizeof(struct unit_mask) *
              node->cluster[cluster_index].core[core_index].num_units);
...
void default_policy(unsigned int number_of_workers, unsigned int number_of_senders,
                   unsigned int number_of_receivers, struct node_mask *node,
...
unsigned int flatten_mask(struct arts_config *config, struct node_mask *node,
                         struct thread_mask **flat) {
  unsigned int i;
  unsigned int j;
  unsigned int k;
  unsigned int total;
  ...
  total = count;
  *flat = (struct thread_mask *)arts_malloc(sizeof(struct thread_mask) * total);
      ...
      for (k = 0; k < node->cluster[i].core[j].num_units; k++) {
        if (node->cluster[i].core[j].unit[k].on) {
          ...
          while (next != NULL) {
            ...
            (*flat)[count].id = count;
            ++count;
            struct unit_thread *temp = next->next;
            arts_free(next);
            next = temp;
          }
        }
...
struct thread_mask *get_thread_mask(struct arts_config *config) {
  ...
  struct thread_mask *flat;
  init_topology();
  struct node_mask *node = get_node_mask();
...
void print_topology(struct node_mask *node) {
  ...
  for (i = 0; i < node->num_clusters; i++) {
    ...
    for (j = 0; j < node->cluster[i].num_cores; j++) {
      ...
      for (k = 0; k < node->cluster[i].core[j].num_units; k++) {
        struct unit_mask *unit = &node->cluster[i].core[j].unit[k];
        struct unit_thread *temp = unit->listHead;
        while (temp != NULL) {
...
void default_policy(unsigned int number_of_workers, unsigned int number_of_senders,
                   unsigned int number_of_receivers, struct unit_mask *flat,
                   unsigned int num_cores, struct arts_config *config) {
...
unsigned int flatten_mask(struct arts_config *config, unsigned int num_cores,
                         struct unit_mask *unit, struct thread_mask **flat) {
  ...
  *flat = (struct thread_mask *)arts_calloc(maskSize, sizeof(struct thread_mask));
...
struct thread_mask *get_thread_mask(struct arts_config *config) {
  ...
  unit = arts_calloc(core_count, sizeof(struct unit_mask));
...
void destroy_thread_mask(struct thread_mask *mask) { arts_free(mask); }
