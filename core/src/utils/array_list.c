...
arts_array_list_element *arts_new_array_list_element(uint64_t start,
                                              size_t element_size,
                                              size_t array_length) {
  arts_array_list_element *ret = (arts_array_list_element *)arts_malloc(
      sizeof(arts_array_list_element) + (element_size * array_length));
  ret->start = start;
  ret->next = NULL;
...
void arts_delete_array_list(arts_array_list_t *a_list) {
  arts_array_list_element *trail;
  arts_array_list_element *current = a_list->head;
  while (current) {
    trail = current;
...
void *arts_get_from_array_list(arts_array_list_t *a_list, uint64_t index) {
  if (a_list) {
    ...
    if (index < a_list->index) {
      ...
      // Slow Path
      arts_array_list_element *node = a_list->head;
