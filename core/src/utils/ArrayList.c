...
artsArrayListElement *artsNewArrayListElement(uint64_t start,
                                              size_t elementSize,
                                              size_t arrayLength) {
  artsArrayListElement *ret = (artsArrayListElement *)artsMalloc(
      sizeof(artsArrayListElement) + elementSize * arrayLength);
  ret->start = start;
  ret->next = NULL;
...
artsArrayList *artsNewArrayList(size_t elementSize, size_t arrayLength) {
  ...
  ret->head = ret->current =
      artsNewArrayListElement(0, elementSize, arrayLength);
  ret->index = 0;
  ret->lastRequest = 0;
...
uint64_t artsPushToArrayList(artsArrayList *aList, void *element) {
  ...
  if (!(aList->index % aList->arrayLength) && aList->index) {
    if (!aList->current->next)
      aList->current->next =
          artsNewArrayListElement(aList->current->start + aList->arrayLength,
                                  aList->elementSize, aList->arrayLength);
    aList->current = aList->current->next;
  }
  uint64_t offset = aList->index - aList->current->start;
  void *ptr =
      (void *)((char *)aList->current->array + offset * aList->elementSize);
...
uint64_t artsLengthArrayList(artsArrayList *aList) { return aList->index; }
  ...
  if (aList) {
    ...
    if (index < aList->index) {
      ...
      if (aList->index < aList->arrayLength) {
        aList->lastRequestPtr =
            (void *)((char *)aList->head->array + index * aList->elementSize);
        return aList->lastRequestPtr;
      }
      ...
      if (node) {
        ...
        aList->lastRequestPtr =
            (void *)((char *)node->array + offset * aList->elementSize);
        return aList->lastRequestPtr;
      }
    }
  }
      ...
      if (iter->current) {
        ret =
            (void *)((char *)iter->current->array +
                     (iter->index - iter->current->start) * iter->elementSize);
        iter->index++;
      }
    }
  }
