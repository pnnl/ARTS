...
arts_queue *arts_new_queue() {
  arts_queue *queue = (arts_queue *)arts_calloc_align(1, sizeof(arts_queue), 128);
  ring_queue *rq = (ring_queue *)arts_calloc_align(1, sizeof(ring_queue), 128);
  init_ring(rq);
  queue->head = queue->tail = rq;
  return queue;
}

void fix_state(ring_queue *rq) {
  uint64_t t;
  uint64_t h;
  uint64_t n;
  while (1) {
...
int close_crq(ring_queue *rq, const uint64_t t, const int tries) {
  if (tries < 10) {
...
void enqueue(OBJECT arg, arts_queue *queue) {
  ring_queue *nrq;
  int try_close = 0;
  while (1) {
    ring_queue *rq = queue->tail;
    ring_queue *next = rq->next;
    ...
    alloc:
      //            ARTS_INFO("Allocing!");
      nrq = (ring_queue *)arts_malloc_align(sizeof(ring_queue), 128);
      init_ring(nrq);
    ...
    ring_node *cell = &rq->array[t & (RING_SIZE - 1)];
    STORE_PREFETCH(cell);
...
OBJECT dequeue(arts_queue *queue) {
  while (1) {
    ring_queue *rq = queue->head;
    ring_queue *next;
    uint64_t h = FAA64(&rq->head, 1);
    ring_node *cell = &rq->array[h & (RING_SIZE - 1)];
    STORE_PREFETCH(cell);
