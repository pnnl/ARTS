...
struct arts_config *arts_global_message_table;
unsigned int ports;
// SOCKETS!
...
void arts_remote_set_message_table(struct arts_config *table) {
...
void arts_server_fix_ib_names(struct arts_config *config) {
#ifdef __linux__
  const char *hostname_formats[] = {"ib-%s",    "ib%s",     "ib.%s",
                                   "%s-ib",    "%sib",     "%s.ib",
                                   "%s-ib.ib", "%s.ibnet", NULL};
  const int request_count = config->table_length * 8;
  ...
  for (int i = 0; i < request_count; i++) {
    ...
    if (!error) {
      ...
      char *new_hostname =
          (char *)arts_malloc(strlen(gaicb_requests[i].ar_name) + 1);
      strcpy(new_hostname, gaicb_requests[i].ar_name);
...
bool arts_server_set_ip(struct arts_config *config) {
  ...
  for (int i = 0; i < config->table_length; i++) {
    result = hostname_to_ip(config->table[i].ip_address, ip_list + (100 * i));
  ...
  if (config->net_interface) {
    ...
    if (!hostname_to_ip(local_hostname, local_default_ip)) {
      ...
      if (!found_iface) {
        ...
        uint32_t offset =
            ntohl(iface_addr.s_addr) - ntohl(default_addr.s_addr);
        ...
        if (offset != 0) {
          ...
          for (int i = 0; i < config->table_length; i++) {
            struct in_addr addr;
            char old_ip[100];
            inet_pton(AF_INET, ip_list + (100 * i), &addr);
            inet_ntop(AF_INET, &addr, old_ip, sizeof(old_ip));
            addr.s_addr = htonl(ntohl(addr.s_addr) + offset);
            inet_ntop(AF_INET, &addr, ip_list + (100 * i), 100);
            ARTS_INFO("  node %d: %s -> %s", i, old_ip, ip_list + 100 * i);
          }
        } else {
  ...
  if (arts_rank_env) {
    arts_global_rank_id = (unsigned int)atoi(arts_rank_env);
  ...
  if (slurm_proc_id) {
    arts_global_rank_id = (unsigned int)atoi(slurm_proc_id);
    config->my_rank = arts_global_rank_id;
    arts_global_rank_count = config->table_length;
    return true;
  }
    ...
    for (ifa = ifap; ifa && !found; ifa = ifa->ifa_next) {
      if (ifa->ifa_addr->sa_family == AF_INET) {
        ...
        for (int i = 0; i < config->table_length && !found; i++) {
          if (strcmp(addr, ip_list + (100 * i)) == 0) {
            found = true;
            config->my_rank = i;
...
void arts_ll_server_setup(struct arts_config *config) {
...
void arts_ll_server_shutdown() {
  int count = arts_global_message_table->table_length;
...
unsigned int arts_remote_get_my_rank() { return arts_global_message_table->my_rank; }
...
static inline bool arts_remote_connect(int rank, unsigned int port) {
  ...
  if (!remote_connection_alive[(rank * ports) + port]) {
    int res = RCONNECT(
        ...
        (struct sockaddr *)(remote_server_send_list + (rank * ports) + port),
    ...
    if (res < 0) {
      ...
      while (RCONNECT(remote_socket_send_list[(rank * ports) + port],
                      (struct sockaddr *)(remote_server_send_list + (rank * ports) +
                                          port),
                      sizeof(struct sockaddr_in)) < 0) {
        if (++retry_count >= max_retries) {
          struct sockaddr_in *addr = remote_server_send_list + (rank * ports) + port;
          ARTS_INFO("arts_remote_connect: Failed to connect to rank %d port %d after %d retries (target %s:%d, errno=%d: %s)",
                    rank, port, max_retries,
...
// inline int arts_actual_send(char * message, unsigned int length, int rank, int
// port)
uint64_t arts_actual_send(char *message, uint64_t length, int rank, int port) {
  ...
  while (length != 0 && res >= 0) {
    ...
    if (iterations > 1000000) {
      ARTS_INFO("arts_actual_send: stuck in loop, res=%d, length=%lu, total=%lu, errno=%d",
  ...
  if (res < 0) {
    if (errno != EAGAIN) {
      struct arts_remote_packet *pk = (struct arts_remote_packet *)message;
      ARTS_INFO(
...
bool arts_remote_setup_incoming() {
  ...
  int in_port = (my_node_port != 0) ? my_node_port : arts_global_message_table->port;
  socklen_t s_length = sizeof(struct sockaddr);
  int count = (arts_global_message_table->table_length - 1);

  remote_socket_recieve_list =
      (int *)arts_malloc(sizeof(int) * (count + 1) * ports);
  ...
  for (i = 0; i < arts_global_message_table->ports; i++) {
    local_socket_recieve[i] =
        arts_get_socket_listening(&local_server_addr[i], in_port + i);
...
void arts_remote_setup_outgoing() {
  ...
  int j;
  int k;
  int out_port = arts_global_message_table->port;
  ...
  remote_socket_send_list = (int *)arts_malloc(sizeof(int) * count * ports);
  remote_socket_send_lock_list =
      (volatile unsigned int *)arts_calloc(count * ports, sizeof(int));
  remote_server_send_list = (struct sockaddr_in *)arts_calloc(
      count * ports, sizeof(struct sockaddr_in));
  remote_connection_alive = (bool *)arts_calloc(count * ports, sizeof(bool));
  ...
  for (i = 0; i < count; i++) {
    ...
    ARTS_INFO("arts_remote_setup_outgoing: node %d ip_list='%s' port=%u", i, ip_list + 100 * i, target_port);

    for (j = 0; j < ports; j++) {
      remote_socket_send_list[(i * ports) + j] =
          arts_get_socket_outgoing(remote_server_send_list + (i * ports) + j,
                                target_port + j, inet_addr(ip_list + (100 * i)));
}
  }
}
...
bool max_out_buffs(unsigned int ignore) {
  ...
  int64_t res;
  int64_t res2;
  struct arts_remote_packet *packet;
  // ARTS_INFO("MAX");
  res = RPOLL(poll_incoming + thread_start, thread_stop - thread_start, time_out);
  unsigned int pos;
    ...
    for (int i = thread_start; i < thread_stop; i++) {
      ...
      if (i != ignore && poll_incoming[i].revents & POLLIN) {
        ...
        if (re_recieve_res[pos] == 0) {
          packet = (struct arts_remote_packet *)bypass_buf[pos];
...
bool arts_server_try_to_receive(char **in_buffer, int *in_packet_size,
                            volatile unsigned int *remote_steal_lock) {
  int i;
      ...
      for (i = thread_start; i < thread_stop; i++) {
        ...
        if (poll_incoming[i].revents & POLLIN) {
          ...
          if (res > 0) {
            ...
            while (res > 0) {
              while (res < sizeof(struct arts_remote_packet)) {
                if (bypass_buf[pos] != (char *)packet) {
                  memmove(bypass_buf[pos], packet, res);
                  packet = (struct arts_remote_packet *)bypass_buf[pos];
                }
              ...
              if (bypass_packet_size[pos] < packet->size) {
                ...
                packet =
                    (struct arts_remote_packet *)(next_buf +
                                                (((char *)packet) -
                                                 (bypass_buf[pos])));
              ...
              res -= packet->size;
              packet =
                  (struct arts_remote_packet *)(((char *)packet) + packet->size);
            }
          } else if (res == -1) {
            arts_shutdown();
            arts_runtime_stop();
...
void arts_server_ping_pong_test_recieve(char *in_buffer, int in_packet_size) {
  ...
  int steal_handler_thread = 0;
  int pos;
  struct arts_remote_packet *packet = (struct arts_remote_packet *)buf;
  int count = arts_global_message_table->table_length - 1;
  fd_set temp_set;
  int time_out = 100;
  struct timeval sel_timeout;
  ...
  while (!recieved) {
    ...
    for (i = 0; i < count; i++) {
      if (poll_incoming[i].revents & POLLIN) {
        packet = (struct arts_remote_packet *)buf;
        ...
        if (res > 0) {
          while (res > 0) {
            ...
            if (packet->message_type == ARTS_REMOTE_PINGPONG_TEST_MSG) {
              recieved = true;
              ARTS_METRICS_TRIGGER_EVENT(ARTS_METRIC_NETWORK_RECIEVE_BW, ARTS_METRIC_THREAD,
                                      packet->size);
