...
void arts_remote_launcher_ssh_startup_processes(
    struct arts_remote_launcher *launcher) {
  unsigned int argc = launcher->argc;
  char **argv = launcher->argv;
  struct arts_config *config = launcher->config;
  ...
  for (k = start_node + 1; k < config->table_length + start_node; k++) {
    i = k % config->table_length;
