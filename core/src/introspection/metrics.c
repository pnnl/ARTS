...
static arts_performance_unit *get_metric(arts_metric_type_t type,
...
//   arts_performance_unit *metric = get_metric(metricType, level);
//   if (metric) {
//     if (start)
...
//       arts_calloc(ARTS_METRIC_LAST_TYPE, sizeof(arts_performance_unit));
//   for (unsigned int j = 0; j < ARTS_METRIC_LAST_TYPE; j++) {
...
static bool metric_try_lock(arts_metric_level_t level, arts_performance_unit *metric) {
...
static void metric_lock(arts_metric_level_t level, arts_performance_unit *metric) {
  if (level == ARTS_METRIC_THREAD) {
    return;
}
...
static void metric_unlock(arts_performance_unit *metric) { metric->lock = 0U; }

static arts_performance_unit *get_metric(arts_metric_type_t type,
                                      arts_metric_level_t level) {
  arts_performance_unit *metric = NULL;
  if (inspector) {
    switch (level) {
...
//   arts_performance_unit *metric = get_metric(type, level);
...
static void internal_update_max(arts_metric_level_t level,
                              arts_performance_unit *metric, uint64_t total) {
...
static void set_thread_metric(arts_metric_type_t type, uint64_t value) {
  ...
  arts_performance_unit *metric = get_metric(type, ARTS_METRIC_THREAD);
  if (metric) {
    bool shot = true;
    ...
    if (line[0] != '#') {
      ...
      if (metric_index < ARTS_METRIC_LAST_TYPE) {
        for (unsigned int i = 0; i < ARTS_METRICLEVELS; i++) {
          ...
          param_read += sscanf(&line[offset], "%" SCNu64 "",
        ...
        for (unsigned int i = 0; i < ARTS_METRICLEVELS; i++) {
          ...
          param_read +=
              sscanf(&line[offset], "%" SCNu64 "", &time_window[metric_index][i]);
          (void)sscanf(&line[offset], "%s", temp);
          offset += strlen(temp);
          ARTS_DEBUG("temp: %s %u %u %u %" PRIu64 "\n", temp, metric_index, i,
        ...
        for (unsigned int i = 0; i < ARTS_METRICLEVELS; i++) {
          ...
          param_read +=
              sscanf(&line[offset], "%" SCNu64 "", &max_total[metric_index][i]);
          (void)sscanf(&line[offset], "%s", temp);
          offset += strlen(temp);
          ARTS_DEBUG("temp: %s %u %u %u %" PRIu64 "\n", temp, metric_index, i,
