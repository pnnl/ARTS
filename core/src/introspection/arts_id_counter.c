...
static inline uint64_t arts_get_time_ns() {
  struct timespec ts;
...
arts_id_find_slot(arts_id_metrics_t *table, uint64_t arts_id, uint64_t *collisions) {
  ...
  if (!table[idx].valid) {
    memset(&table[idx], 0, sizeof(arts_id_metrics_t));
...
void arts_id_reduce_hash_tables(arts_id_hash_table_t *dest, const arts_id_hash_table_t *src) {
  ...
  for (uint32_t i = 0; i < ARTS_ID_HASH_SIZE; i++) {
    ...
    arts_id_metrics_t *dest_slot =
        arts_id_find_slot(dest->edt_metrics, arts_id, &dest->edt_collisions);
  ...
  for (uint32_t i = 0; i < ARTS_ID_HASH_SIZE; i++) {
    ...
    arts_id_metrics_t *dest_slot =
        arts_id_find_slot(dest->db_metrics, arts_id, &dest->db_collisions);
