...
unsigned int check_and_set(uint64_t *mask, unsigned int index) {
...
uint64_t process_block(uint64_t index) {
...
void visit_node(uint32_t paramc, const uint64_t *paramv, uint32_t depc,
  ...
  local_count += process_block(index);
  uint64_t next_index = (num_blocks - 1) - index;
  if (next_index != index) {
    local_count += process_block(next_index);
  }
