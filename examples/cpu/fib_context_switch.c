...
void fib(uint32_t paramc, const uint64_t *paramv, uint32_t depc,
  ...
  if (num >= 2) {
    arts_ticket_t ctx = arts_get_context_ticket();
    unsigned int count = 2;

    int *x_ptr = &x;
    int *y_ptr = &y;
    arts_guid_t x_guid =
        arts_allocate_local_buffer((void **)&x_ptr, sizeof(int), 1, NULL_GUID);
    arts_guid_t y_guid =
        arts_allocate_local_buffer((void **)&y_ptr, sizeof(int), 1, NULL_GUID);
  ...
  if (result_guid) {
    arts_set_buffer(result_guid, &sum, sizeof(int));
