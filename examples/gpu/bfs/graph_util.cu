...
__device__ vertex_t *getRowPtrGpu(csr_graph_t *csr) {
  return (vertex_t *)(csr + 1);
}

__device__ vertex_t *getColPtrGpu(csr_graph_t *csr) {
  return getRowPtrGpu(csr) + csr->num_local_vertices + 1;
}
...
__device__ void getNeighborsGpu(csr_graph_t *csr, vertex_t v, vertex_t **out,
                                graph_sz_t *neighborcount) {
  vertex_t *row_indices = getRowPtrGpu(csr);
  vertex_t *columns = getColPtrGpu(csr);
  // get the local index for the vertex
  local_index_t i = getLocalIndexGpu(v, csr);
...
void getProperties(char *filename, unsigned int *num_verts,
                   unsigned int *num_edges) {
  FILE *fp = fopen(filename, "r");
  if (fp) {
    unsigned int nonZero = 0;
    fscanf(fp, "%u %u %u", num_verts, &nonZero, num_edges);
    ARTS_PRINTF("Verts: %u Edges: %u NonZero: %u\n", *num_verts, *num_edges, nonZero);
    fclose(fp);
  } else
