...
void create_first_round(uint32_t paramc, const uint64_t *paramv, uint32_t depc,
  ...
  arts_guid_t first_search_frontier_guid =
      arts_guid_reserve(ARTS_DB_GPU_READ, 0);
  ...
  if (first_search_frontier[0] > GPU_THRESHOLD) {
    ...
    bfs_guid =
        arts_edt_create(cpu_bfs, 1, &next_level, 4,
                        &(arts_hint_t){.route = arts_get_current_node()});
...
void launch_sort(uint32_t paramc, const uint64_t *paramv, uint32_t depc,
  ...
  for (unsigned int j = 0; j < arts_get_total_nodes(); j++) {
    for (uint64_t i = 0; i < arts_get_total_gpus(); i++) {
      arts_edt_create_gpu_lib_direct(thrust_sort, j, i, 2, args, 0, grid,
                                     threads);
    }
      ...
      if (size_per_bound[i]) {
        ...
        *new_search_frontier = size_per_bound[i];
        ...
        } else // Create CPU EDT
        {
          ...
          edt_guids_to_launch_bfs[i] = arts_edt_create(
              cpu_bfs, 1, &next_level, 4, &(arts_hint_t){.route = rank});
        }
        ...
        for (unsigned int j = 0; j < PARTS; j++) {
          if (guid_to_signal[j]) {
            ...
            arts_signal_edt(guid_to_signal[j], 1,
                            next_search_frontier_addr_guid[rank],
                            ARTS_DB_WRITE);
            total_new_bfs++;
          }
        }
...
void init_node(uint32_t paramc, const uint64_t *paramv, uint32_t depc,
  ...
  distribution = init_block_distribution_block(num_verts, num_edges, PARTS,
                                               ARTS_DB_GPU_READ);
