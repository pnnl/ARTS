...
All five SAR rows share these sources; `sar_medium` is the build that compiles in
`ocr/medium/Parameters.h` and links the `medium` dataset.
...
image into `refImage` and zeroes `curImage`, and `BackProj` fans out 256
...
`expect: '1991'` is a hard equality.  Arithmetic is real — 1.10 G complex
...
`sar_medium` takes **no command-line arguments**; the catalog's `args: []` is
...
| `RAG_Ix`, `RAG_Iy` | image pixels | 512 × 512 | ✗ `ocr/medium/Parameters.h` |
| `RAG_P1`, `RAG_S1` | pulses per image; complex samples per pulse | 4200, 4000 | ✗ same; must match the linked dataset |
| `RAG_Sx`, `RAG_Sy` | spotlight subimage | 512 × 512, so `TF = Ix/Sx = 1` | ✗ same; `TF > 1` aborts ("not yet supported") |
| `RAG_Nc` | affine control-point budget | 64 → `N = ⌊√Nc⌋ = 8` per axis | ✗ same |
...
With `Ix = Iy = 512`, tile edge `B = 32`, `N = ⌊√Nc⌋ = 8`, and the three
tiled window grids `T_bp = ⌈Ix/B⌉·⌈Iy/B⌉ = 256`,
`T_ccd = ⌈(Iy−Ncor+1)/B⌉² = 256`,
`T_cfar = ⌈(Iy−Ncor−Ncfar+2)/B⌉² = 256`:
...
| tile EDTs | `2·T_bp` backprojection + `N²` correlation + `T_bp` resample + `T_ccd` CCD + `T_cfar` CFAR | 512 + 64 + 256 + 256 + 256 | — |
| global DBs | 14, all in `mainEdt` | 14 | `X` 128.2 MiB, `curImage`/`refImage` 2.00 MiB each, `corr_map` 2.96 MiB, `Y` 2.68 MiB, `Pt`/`Tp`/axis vectors, `file_args` 4 KiB, four parameter blocks (12–88 B) |
| `Affine` DBs | `5 + N²` | 69 | `Fx`/`Fy` (`Nc·4`), `A` (`Nc·32`), `output` 2.00 MiB, 56 B per control point |
| per-task scratch | `2·N²` correlation windows (1.8 KiB) + `6·T_bp` backprojection (8.4 KiB tile copy, two 32-vectors) + `T_cfar` clutter windows (2.7 KiB) + 5 fixed | 1925 | — |
| EDT templates | 21 per rank | 21 | into a fixed `templateList[25]`, bounds-checked on every claim |

Totals: `18 + 3·T_bp + N² + T_ccd + T_cfar` = **1362 EDTs**,
`24 + 3·N² + 6·T_bp + T_cfar` = **2008 DBs**, and **20 events regardless of
...
Live set ≈ 145 MiB, dominated by `X` and the three image-shaped blocks — and it
...
The binary additionally carries 256 MiB of `.rodata` — the linked dataset as
...
`T_bp = 256`, `N² = 64`, `T_ccd = 256`, `T_cfar = 256`.
...
|-----------|-----------|------------|------------------------|
| `X`, `Pt`, `Tp` | `ReadData` (1) | `backproject_async` | 256 |
| `curImage` | `backproject_async` (256), `post_Affine` (1) | `affine_async_1`, `affine_async_2`, `ccd_async` | 256 |
| `refImage` | `FormImage` (1) | `affine_async_1`, `ccd_async` | 64 |
| `output` | `affine_async_2` (256) | `post_Affine` | 1 |
| `affine_params`, `Fx`, `Fy`, `A` | `affine_async_1` (64) | `post_affine_async_1/2` | 1 |
| `corr_map` | `ccd_async` (256) | `cfar_async` | 256 |
| `Y`, `Nd` | `cfar_async` (256) | `post_CFAR` | 1 |
...
and OCR `RW` is per-node exclusive — 256 backprojection tiles all take
`curImage` RW, 256 CCD tiles `corr_map`, 256 CFAR tiles `Y` and `Nd`,
64 correlation tasks `affine_params`/`Fx`/`Fy`/`A`.  Tiles are round-robin
...
point: 512 exclusive acquisitions of a 2.00 MiB block in backprojection alone,
over disjoint pixel ranges — pure protocol cost, not an algorithmic
dependence.  And the fan-out is genuinely broadcast-shaped: `X` (128.2 MiB) read
concurrently by all 256 tiles of a round, `curImage`/`refImage` by up to
256 tasks, and the 88-byte `image_params` by essentially every task —
...
2. `refReadData` → `refFormImage` — blit 128.2 MiB of pulse data out of `.rodata`,
   zero `curImage` (2.00 MiB).  Both serial.
3. `BackProj` → **256 `backproject_async`** — the heaviest stage; each tile
   sweeps all 4200 pulses over `32×32` pixels, ≈ 4.3 M complex MACs.
4. `post_FormImage` → `ReadData` → `FormImage` (which now really does the
   `curImage → refImage` copy, 2.00 MiB) → `BackProj` → **256 tiles** again.
5. `Affine` → **64 `affine_async_1`**, one per control point,
   `(2·Rc+1)²·Sc²` ≈ 245 k operations each.
6. `post_affine_async_1` — serial `A'A`/`A'F` accumulation and two 6×6
   Gaussian eliminations → **256 `affine_async_2`** resampling tiles.
7. `post_affine_async_2` → `post_Affine` — serial full-image copy
   `output → curImage` (2.00 MiB).
8. `CCD` → **256 `ccd_async`**; `CFAR` → **256 `cfar_async`**;
   `post_CFAR` writes `Nd` lines; `post_main` shuts down.

Max parallel width is `max(T_bp, N², T_ccd, T_cfar)` = **256**, but what
bounds how much machine a rung keeps busy is the *narrowest* wide stage,
**64**.  Serial bottlenecks in cost order: the two `ReadData` blits, the
...
`sar_medium_hinted` family compiles it in — the catalog's `hinted: true`).  So:
**EDTs** → the shim passes `ARTS_HINT_ANY_RANK` → runtime round-robin, and all
1344 tile tasks plus every stage head land on arbitrary ranks; **DBs** →
...
data point.  `sar_medium` is the third rung.
...
explicit `timeout: 120`.  `sar_medium` is the first rung with a realistic per-tile grain (4.3 M MACs, coming from `P1·S1` = 4200×4000 rather than from image size) and the first whose working set leaves cache — a good 1–2 node point, and the rung to use when `sar_large`'s 3600 correlation tasks would dominate the profile.
...
1 however wide the stage — extra nodes buy 2.00 MiB-sized transfers, not
parallelism, and the strong-scaling curve should be expected to bend or invert.
Memory never constrains the choice on this machine (~145 MiB live plus 256 MiB
