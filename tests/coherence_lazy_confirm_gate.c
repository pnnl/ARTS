...
/// @file coherence_lazy_confirm_gate.c
/// @brief Exploit + regression for the LAZY ownership-confirm gate.
///
...
/// 1 = all reads fresh; -1 = a stale read was observed.
static atomic_int g_result = 1;
/// Set once the loop finishes so the watchdog exits quietly on success.
static atomic_int g_done = 0;

/// Watchdog: a stranded gated waiter would hang the loop.  Fail loudly rather
/// than hang so the process self-kills and releases its ports cleanly, well
/// below the ctest TIMEOUT (120s).
static void *wd_thread(void *arg) {
  ...
  (void)fprintf(stderr, "HANG: coherence_lazy_confirm_gate did not finish\n");
  (void)fflush(stderr);
  _exit(1);
}
...
void reader_edt(uint32_t paramc, const uint64_t *paramv, uint32_t depc,
  ...
  (void)paramc;
  (void)depc;
  unsigned int *d = (unsigned int *)depv[0].ptr; /* slot 0 = DB RO */
  unsigned int expect = (unsigned int)paramv[0];
  if (d == NULL || d[0] != expect) {
    atomic_store(&g_result, -1);
...
int main(int argc, char **argv) {
  arts_rt(argc, argv);
  if (arts_get_current_rank() == 0 && atomic_load(&g_result) != 1) {
    (void)fprintf(stderr,
                  "FAIL: coherence_lazy_confirm_gate observed a stale read\n");
    return 1;
  }
  return 0;
}
