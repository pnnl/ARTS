...
/// @file mrnew_publish_before_decrement.c
...
/// BEFORE the decrement, and arts_db_send_ownership_response re-arms the field
...
///
/// MRNEW-only, both timings (the publish-before-decrement is identical in the
/// INVALIDATE handler of eager.c and lazy.c).  Self-skips elsewhere; 1n is a
...
int main(void) {
  printf("SKIP mrnew_publish_before_decrement: MRNEW-only\n");
  return 0;
}
...
static void check_edt(uint32_t paramc, const uint64_t *paramv, uint32_t depc,
  ...
  if (d == NULL || d[0] != expect) {
    (void)fprintf(stderr,
                  "FAIL mrnew_publish_before_decrement: expected %u got %u\n",
                  expect, d ? d[0] : 0u);
    arts_abort(1);
  }
}
...
void main_edt(uint32_t paramc, const uint64_t *paramv, uint32_t depc,
  ...
  arts_printf("=== mrnew_publish_before_decrement ===\n");
  ...
  arts_printf("PASS mrnew_publish_before_decrement sum=%u\n", total);
  arts_shutdown();
}
