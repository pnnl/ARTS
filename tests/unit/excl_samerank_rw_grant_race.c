 ...
 *
 * excl_samerank_rw_grant_race — EXCL-config-specific reproducer for the
 ...
 * Under the EXCL protocol the home grants one rank-granular GRANT and the
 ...
 *
 * The test is CORRECT-AND-FAILING by design under EXCL while the bug is
...
#include <stdio.h>
int main(void) {
  printf("SKIP excl_samerank_rw_grant_race: EXCL-only\n");
  return 0;
}
#else
...
static void verify_edt(uint32_t paramc, const uint64_t *paramv, uint32_t depc,
  ...
  printf("excl_samerank_rw_grant_race: %d iters x %d RW = %d grants — PASS\n",
...
void main_edt(uint32_t paramc, const uint64_t *paramv, uint32_t depc,
  ...
  arts_printf("=== excl_samerank_rw_grant_race (%d iters x %d RW) ===\n",
              M_ITERS, N_CONSUMERS);
