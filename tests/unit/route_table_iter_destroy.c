...
void main_edt(uint32_t paramc, const uint64_t *paramv, uint32_t depc,
  ...
  /* The destroyed DB entries are reclaimed by the runtime's own route-table GC
   * at shutdown (arts_clean_up_dbs).  We deliberately do NOT call
   * arts_clean_up_route_table here: it is a whole-table teardown primitive that
   * releases EVERY install ref — including this currently-running EDT's own cb,
   * which the scheduler still reads after this body returns (use-after-free).
   * Iterate once more to confirm the marked entries no longer iterate. */
  print_rt("After Mark Delete (re-iterate)");
