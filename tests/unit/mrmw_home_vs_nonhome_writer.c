 ...
 *
 * T104 — MRMW home-rank writer concurrent with non-home writer: no buffer
 *        corruption, only a (legal, DB-DRF) lost update.
 *
 ...
 * DB-DRF two concurrent writers race and the last install wins (lost update is
 ...
 *   real corruption FAIL; either uniform value is a legal DB-DRF outcome.
 *
 * config_specific: MRMW only — the concurrent cross-rank RW with no ownership
 * serialization is the DB-DRF contract; ownership protocols serialize writers.
 */
...
void main_edt(uint32_t paramc, const uint64_t *paramv, uint32_t depc,
  ...
  (void)depc;
  (void)depv;
  arts_printf("SKIP mrmw_home_vs_nonhome_writer: MRMW-only\n");
  arts_shutdown();
}
#else
...
static void verifier_edt(uint32_t paramc, const uint64_t *paramv, uint32_t depc,
  ...
  uint8_t *p = (uint8_t *)depv[0].ptr;
  if (p == NULL) {
    arts_printf("mrmw_home_vs_nonhome_writer: FAIL got NULL buffer\n");
    arts_abort(1);
  }
  uint8_t v = p[0];
  for (size_t i = 1; i < DB_SIZE; i++) {
    if (p[i] != v) {
      arts_printf(
          "mrmw_home_vs_nonhome_writer: FAIL torn buffer at %zu (%u vs %u)\n",
          i, (unsigned)p[i], (unsigned)v);
      arts_abort(1); /* corruption: a real bug, never a legal DB-DRF outcome */
    }
  }
  if (v != FILL_HOME && v != FILL_REMOTE) {
    arts_printf("mrmw_home_vs_nonhome_writer: FAIL unexpected value %u\n",
                (unsigned)v);
    arts_abort(1);
  }
}
...
void main_edt(uint32_t paramc, const uint64_t *paramv, uint32_t depc,
  ...
  if (arts_get_total_ranks() < 2) {
    arts_printf("SKIP mrmw_home_vs_nonhome_writer: requires 2+ ranks\n");
    arts_shutdown();
    return;
  }
  ...
  arts_printf("PASS mrmw_home_vs_nonhome_writer: %u rounds, no corruption\n",
              ROUNDS);
  arts_shutdown();
}
#endif
