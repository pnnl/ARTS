...
/// @file rcu_eager_vs_lazy_divergence.c
/// @brief Differential: same RW->RW->RO transfer scenario, asserted correct
///        under BOTH RCU timings (EAGER and LAZY).
///
/// The two RCU timing TUs (eager.c / lazy.c) drive the SAME owner->owner
...
///
/// RCU-only (eager.c/lazy.c are not compiled under WRF_RCU/RWLOCK).  Self
...
int main(void) {
  printf("SKIP rcu_eager_vs_lazy_divergence: RCU-only\n");
  return 0;
}
...
static void rd_edt(uint32_t paramc, const uint64_t *paramv, uint32_t depc,
  ...
  if (d == NULL || d[0] != expect) {
    (void)fprintf(
        stderr, "FAIL rcu_eager_vs_lazy_divergence: expected 0x%x got 0x%x\n",
        expect, d ? d[0] : 0u);
    arts_abort(1);
  }
}
...
void main_edt(uint32_t paramc, const uint64_t *paramv, uint32_t depc,
...
#endif
  arts_printf("=== rcu_eager_vs_lazy_divergence (%s) ===\n", timing);
  ...
  arts_printf("PASS rcu_eager_vs_lazy_divergence (%s) %u steps\n", timing,
              STEPS);
  arts_shutdown();
}
