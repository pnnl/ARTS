...
///
/// ownership/LOCK only (RW is serialized; the cursor + rw_secure are live).
/// MRMW does not serialize RW (Pass-2 cursor is a no-op) -> self-skip.  Runs in
...
void main_edt(uint32_t paramc, const uint64_t *paramv, uint32_t depc,
  ...
  arts_printf("SKIP db_rw_secure_double_fire: MRMW does not serialize RW\n");
  arts_shutdown();
  return;
#else
