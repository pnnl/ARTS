 ...
 *
 * T105 — WRF_RCU sentinel-DB snapshot serve: a zero-size DB acquired cross-rank
 ...
 * (census 10, hazard #4).  Under WRF_RCU a non-home acquire parks on a
 ...
 *   (b) HOME_RECV pre-WRITEBACK — a remote acquire arriving before the home
 ...
 *
 * config_specific: WRF_RCU only — the unified-acquire/SNAPSHOT model is WRF_RCU's;
...
void main_edt(uint32_t paramc, const uint64_t *paramv, uint32_t depc,
  ...
  (void)depc;
  (void)depv;
  arts_printf("SKIP wrf_rcu_sentinel_snapshot: WRF_RCU-only\n");
  arts_shutdown();
}
#else
...
static void sentinel_edt(uint32_t paramc, const uint64_t *paramv, uint32_t depc,
  ...
  if (depv[0].ptr != NULL) {
    arts_printf("wrf_rcu_sentinel_snapshot: FAIL mode=%llu non-NULL ptr for "
                "zero-size DB\n",
...
void main_edt(uint32_t paramc, const uint64_t *paramv, uint32_t depc,
  ...
  if (arts_get_total_ranks() < 2) {
    arts_printf("SKIP wrf_rcu_sentinel_snapshot: requires 2+ ranks\n");
    arts_shutdown();
    return;
  }
  ...
  for (unsigned int rnd = 0; rnd < ROUNDS; rnd++) {
    ...
    /* RW acquire on the same non-home rank — also parks under WRF_RCU (unified
  ...
  arts_printf("PASS wrf_rcu_sentinel_snapshot: %u rounds RO+RW, parked readers "
              "woke (NULL ptr)\n",
              ROUNDS);
  arts_shutdown();
}
