...
/// auto-acquire RW; the home rank is the initial IDLE owner.  Under RCU the
...
///
/// RCU-only, both timings (the NO_ACQUIRE creator-home init + first-request
...
int main(void) {
  printf("SKIP rcu_no_acquire: RCU-only\n");
  return 0;
}
...
static void check_edt(uint32_t paramc, const uint64_t *paramv, uint32_t depc,
  ...
  if (d == NULL || d[0] != expect) {
    (void)fprintf(stderr, "FAIL rcu_no_acquire: expected %u got %u\n", expect,
                  d ? d[0] : 0u);
    arts_abort(1);
  }
}
...
void main_edt(uint32_t paramc, const uint64_t *paramv, uint32_t depc,
  ...
  arts_printf("PASS rcu_no_acquire value=%u\n", (unsigned int)expect);
  arts_shutdown();
}
