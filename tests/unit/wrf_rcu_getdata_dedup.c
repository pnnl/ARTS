 ...
 *
 * T103 — WRF_RCU GET_DATA dedup watermark stress: hammer one requester rank with
 *        many concurrent SNAPSHOT_REQUESTs and confirm the dedup never serves
 *        stale NO_DATA.
 *
 * Targets update_cached_version_max (census 10, hazard #2).  Under WRF_RCU every
 * non-home acquire (RO and RW) parks on a SNAPSHOT_REQUEST.  The home serves
 * the canonical buffer the first time, records cached_version[requester],
 ...
 *
 * config_specific: WRF_RCU only — the SNAPSHOT_REQUEST/NO_DATA dedup path is the
 * WRF_RCU acquire model; ownership protocols use a different acquire round.
 */
...
void main_edt(uint32_t paramc, const uint64_t *paramv, uint32_t depc,
  ...
  (void)depc;
  (void)depv;
  arts_printf("SKIP wrf_rcu_getdata_dedup: WRF_RCU-only\n");
  arts_shutdown();
}
#else
...
static void reader_edt(uint32_t paramc, const uint64_t *paramv, uint32_t depc,
  ...
  if (p == NULL) {
    arts_printf(
        "wrf_rcu_getdata_dedup: reader %llu FAIL got NULL (stale NO_DATA)\n",
  ...
  if (sum != expected) {
    arts_printf("wrf_rcu_getdata_dedup: reader %llu FAIL sum=%llu expected=%llu\n",
...
void main_edt(uint32_t paramc, const uint64_t *paramv, uint32_t depc,
  ...
  if (arts_get_total_ranks() < 2) {
    arts_printf("SKIP wrf_rcu_getdata_dedup: requires 2+ ranks\n");
    arts_shutdown();
    return;
  }
  ...
  arts_printf(
      "PASS wrf_rcu_getdata_dedup: %u rounds x %d readers, no stale NO_DATA\n",
      ROUNDS, N_READERS);
  arts_shutdown();
}
#endif
