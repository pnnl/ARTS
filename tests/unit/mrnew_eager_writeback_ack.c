...
/// @file mrnew_eager_writeback_ack.c
/// @brief Exercise the EAGER synchronous WRITEBACK ACK rendezvous as a unit.
///
/// Under EAGER, arts_db_release_rw on a NON-home owner performs a SYNCHRONOUS
/// WRITEBACK to home: it builds a stack-local `sem_t cv`, ships its address
/// over the wire (arts_send_db_writeback), then blocks in await_writeback_ack
/// until arts_handler_db_writeback_ack sem_posts it.  The dispatcher posts the
...
/// stamp a per-step value and RELEASE — each release is a full WRITEBACK->ACK
/// round.  After each remote write, a home RO reader must observe that exact
/// value (home is canonical under EAGER only because the synchronous WRITEBACK
...
///
/// EAGER-only (the synchronous WRITEBACK + stack-sem path exists ONLY in
/// eager.c; lazy.c drops the buffer ref and never blocks on an ACK).  Compiles
/// to a clean skip under any other configuration.  Needs 2+ ranks for a remote
/// owner; at 1n the owner is home (no WRITEBACK) and the chain still passes.
...
#if !defined(ARTS_PROTOCOL_MRNEW) || !defined(ARTS_TIMING_EAGER)

int main(void) {
  printf("SKIP mrnew_eager_writeback_ack: MRNEW+EAGER-only\n");
  return 0;
}
...
static void rd_edt(uint32_t paramc, const uint64_t *paramv, uint32_t depc,
  ...
  if (d == NULL || d[0] != expect) {
    (void)fprintf(stderr,
                  "FAIL mrnew_eager_writeback_ack: expected 0x%x got 0x%x\n",
                  expect, d ? d[0] : 0u);
    arts_abort(1);
  }
}
...
void main_edt(uint32_t paramc, const uint64_t *paramv, uint32_t depc,
  ...
  arts_printf("=== mrnew_eager_writeback_ack ===\n");

  unsigned int nranks = arts_get_total_ranks();
  unsigned int W = (nranks > 1) ? 1u : 0u; /* remote owner forces WRITEBACK */
  ...
  arts_printf("PASS mrnew_eager_writeback_ack %u steps\n", STEPS);
  arts_shutdown();
}
