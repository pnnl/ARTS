...
/// @file mrnew_baton_invalidate_recheck.c
...
///
/// arts_handler_db_ownership_request pushes the requester onto the home
...
///
/// MRNEW-only, both timings (the baton + request handler are shared
/// ownership.c; the confirm recheck loop exists in both eager.c and lazy.c
...
int main(void) {
  printf("SKIP mrnew_baton_invalidate_recheck: MRNEW-only\n");
  return 0;
}
...
static void check_edt(uint32_t paramc, const uint64_t *paramv, uint32_t depc,
  ...
  if (d == NULL || d[0] != expect) {
    (void)fprintf(stderr,
                  "FAIL mrnew_baton_invalidate_recheck: expected %u got %u\n",
                  expect, d ? d[0] : 0u);
    arts_abort(1);
  }
}
...
void main_edt(uint32_t paramc, const uint64_t *paramv, uint32_t depc,
  ...
  arts_printf("=== mrnew_baton_invalidate_recheck ===\n");
  ...
  arts_printf("PASS mrnew_baton_invalidate_recheck sum=%u\n", total);
  arts_shutdown();
}
