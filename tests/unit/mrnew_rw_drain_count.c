...
///
/// MRNEW-only, both timings (the drain cb + accounting is shared ownership.c;
...
int main(void) {
  printf("SKIP mrnew_rw_drain_count: MRNEW-only\n");
  return 0;
}
...
static void check_edt(uint32_t paramc, const uint64_t *paramv, uint32_t depc,
  ...
  unsigned int *arr = (unsigned int *)depv[0].ptr;
  if (arr == NULL) {
    (void)fprintf(stderr, "FAIL mrnew_rw_drain_count: NULL buffer at check\n");
    arts_abort(1);
  }
  for (unsigned int i = 0; i < N; i++) {
    if (arr[i] != i + 1u) {
      (void)fprintf(stderr,
                    "FAIL mrnew_rw_drain_count: slot %u expected %u got %u\n",
                    i, i + 1u, arr[i]);
      arts_abort(1);
    }
  }
...
void main_edt(uint32_t paramc, const uint64_t *paramv, uint32_t depc,
  ...
  arts_printf("=== mrnew_rw_drain_count ===\n");
  ...
  arts_printf("PASS mrnew_rw_drain_count N=%u rounds=%u\n", N, ROUNDS);
  arts_shutdown();
}
