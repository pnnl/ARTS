...
/// @file mrnew_writer_count_nonneg.c
/// @brief Hammer the MRNEW writer_count non-negativity / +2 drain-guard window
///        under adversarial INVALIDATE(-1) vs OWNERSHIP_RESPONSE(+2) reorder.
///
/// writer_count is the central MRNEW ownership counter: +1 (acquire fast /
...
///
/// This test drives the window the same way coherence_lazy_ownership_reorder
/// does, but as an MRNEW-both-timings gate (EAGER exercises the eager
/// INVALIDATE path; LAZY exercises the TRANSFER+INVALIDATE two-wire reorder).
...
///
/// MRNEW-only; self-skips elsewhere.  At 1n there is no transfer (degenerate
...
int main(void) {
  printf("SKIP mrnew_writer_count_nonneg: MRNEW-only\n");
  return 0;
}
...
static void check_edt(uint32_t paramc, const uint64_t *paramv, uint32_t depc,
  ...
  if (d == NULL || d[0] != expect) {
    (void)fprintf(stderr,
                  "FAIL mrnew_writer_count_nonneg: sum expected %u got %u\n",
                  expect, d ? d[0] : 0u);
    arts_abort(1);
  }
}
...
void main_edt(uint32_t paramc, const uint64_t *paramv, uint32_t depc,
  ...
  arts_printf("=== mrnew_writer_count_nonneg ===\n");
  ...
  arts_printf("PASS mrnew_writer_count_nonneg sum=%u\n", total);
  arts_shutdown();
}
