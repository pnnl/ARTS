 ...
 *
 * lock_destroy_during_acquire — LOCK-config: verify clean destroy AFTER all
 * RW acquirers have quiesced (LOCK protocol).
 *
...
#include <stdio.h>
int main(void) {
  printf("SKIP lock_destroy_during_acquire: LOCK-only\n");
  return 0;
}
#else
...
static void shutdown_edt(uint32_t paramc, const uint64_t *paramv, uint32_t depc,
  ...
  (void)depc;
  (void)depv;
  printf("lock_destroy_during_acquire: %d iters x %d acquirers — PASS\n",
         N_ITERATIONS, N_EDTS);
  arts_shutdown();
}
...
void main_edt(uint32_t paramc, const uint64_t *paramv, uint32_t depc,
  ...
  arts_printf("=== lock_destroy_during_acquire (%d iter, %d acquirers) ===\n",
              N_ITERATIONS, N_EDTS);
