 ...
 *
 * lock_home_grant_d6_d7 — LOCK-config-specific runtime exercise of the home
 * arbiter's D6 (rw→rw chain) and D7 (new RO grants while RW waiting) decision
 * paths (census 11-lock §1 decision table + lock_compute_next / lock_home_grant
 * in lock/home.c).  The pure truth-table is covered by the unit test
 * lock_compute_next; this drives the same transitions through the live runtime.
 *
...
#include <stdio.h>
int main(void) {
  printf("SKIP lock_home_grant_d6_d7: LOCK-only\n");
  return 0;
}
#else
...
static void chain_verify_edt(uint32_t paramc, const uint64_t *paramv,
  ...
  printf("lock_home_grant_d6_d7 D6: rw->rw chain %d — OK\n", N_CHAIN_WRITERS);
}
...
static void shutdown_edt(uint32_t paramc, const uint64_t *paramv, uint32_t depc,
  ...
  (void)depc;
  (void)depv;
  printf("lock_home_grant_d6_d7 D7: RO-extend %d + trail writer — PASS\n",
         N_EXTEND_READERS);
  arts_shutdown();
}
...
void main_edt(uint32_t paramc, const uint64_t *paramv, uint32_t depc,
  ...
  arts_printf("=== lock_home_grant_d6_d7 ===\n");
