...
/// @file mrnew_snapshot_request.c
/// @brief Differential RO snapshot serve: EAGER home-master vs LAZY redirect.
///
/// arts_handler_db_snapshot_request (home side) diverges by timing:
///   - EAGER: home holds the canonical buffer (kept current by synchronous
///     WRITEBACK), serves the snapshot itself, and dedups foreign requesters
///     via update_last_sent_max (NO_DATA when the requester's watermark is
///     current).
///   - LAZY: home does NOT hold data; it records the requester in cached_ranks,
...
///
/// MRNEW-only, runs under both timings (the contrast is the whole point — the
...
int main(void) {
  printf("SKIP mrnew_snapshot_request: MRNEW-only\n");
  return 0;
}
...
static void rd_edt(uint32_t paramc, const uint64_t *paramv, uint32_t depc,
  ...
  if (d == NULL || d[0] != expect) {
    (void)fprintf(stderr,
                  "FAIL mrnew_snapshot_request: expected 0x%x got 0x%x\n",
                  expect, d ? d[0] : 0u);
    arts_abort(1);
  }
}
...
void main_edt(uint32_t paramc, const uint64_t *paramv, uint32_t depc,
...
#endif
  arts_printf("=== mrnew_snapshot_request (%s) ===\n", timing);
  ...
  for (unsigned int r = 0; r < ROUNDS; r++) {
     ...
     * hit the dedup/watermark branch (EAGER update_last_sent_max /
     * LAZY monotonic redirect watermark). */
  ...
  arts_printf("PASS mrnew_snapshot_request (%s) rounds=%u\n", timing, ROUNDS);
  arts_shutdown();
}
