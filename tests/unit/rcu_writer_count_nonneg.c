...
/// @file rcu_writer_count_nonneg.c
/// @brief Hammer the RCU writer_count non-negativity / +2 drain-guard window
///        under adversarial INVALIDATE(-1) vs OWNERSHIP_RESPONSE(+2) reorder.
///
/// writer_count is the central RCU ownership counter: +1 (acquire fast /
...
/// does, but as an RCU-both-timings gate (EAGER exercises the eager
...
///
/// RCU-only; self-skips elsewhere.  At 1n there is no transfer (degenerate
...
int main(void) {
  printf("SKIP rcu_writer_count_nonneg: RCU-only\n");
  return 0;
}
...
static void check_edt(uint32_t paramc, const uint64_t *paramv, uint32_t depc,
  ...
  if (d == NULL || d[0] != expect) {
    (void)fprintf(stderr,
                  "FAIL rcu_writer_count_nonneg: sum expected %u got %u\n",
                  expect, d ? d[0] : 0u);
    arts_abort(1);
  }
}
...
void main_edt(uint32_t paramc, const uint64_t *paramv, uint32_t depc,
  ...
  arts_printf("=== rcu_writer_count_nonneg ===\n");
  ...
  arts_printf("PASS rcu_writer_count_nonneg sum=%u\n", total);
  arts_shutdown();
}
