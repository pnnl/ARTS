...
/// @file mrnew_eager_vs_lazy_divergence.c
/// @brief Differential: same RW->RW->RO transfer scenario, asserted correct
///        under BOTH MRNEW timings (EAGER and LAZY).
///
/// The two MRNEW timing TUs (eager.c / lazy.c) drive the SAME owner->owner
/// ownership transfer with divergent INTERNAL plumbing:
///   - EAGER: arts_handler_db_ownership_response drains pending_rw + runs the
///     newly-granted RW EDT IMMEDIATELY at the OWNERSHIP_RESPONSE (no stale-RO
///     window; home serves RO synchronously).
///   - LAZY: the same RESPONSE only installs the buffer + sentinel and sets
///     ownership_unconfirmed=1; the RW drain is DEFERRED to the CONFIRM_ACK
...
/// pins the contrast as ONE scenario built and run under each timing (each
/// build dir defines exactly one of ARTS_TIMING_EAGER / ARTS_TIMING_LAZY) and
/// asserts the end-to-end correctness that both plumbings must satisfy: a
/// transferred-then-read value is exact.  The pass token records which timing
...
/// must carry the latest buffer in either timing.  Mismatch => arts_abort.
///
/// MRNEW-only (eager.c/lazy.c are not compiled under MRSW/MRMW/LOCK).  Self
...
int main(void) {
  printf("SKIP mrnew_eager_vs_lazy_divergence: MRNEW-only\n");
  return 0;
}
...
static void rd_edt(uint32_t paramc, const uint64_t *paramv, uint32_t depc,
  ...
  if (d == NULL || d[0] != expect) {
    (void)fprintf(
        stderr, "FAIL mrnew_eager_vs_lazy_divergence: expected 0x%x got 0x%x\n",
        expect, d ? d[0] : 0u);
    arts_abort(1);
  }
}
...
void main_edt(uint32_t paramc, const uint64_t *paramv, uint32_t depc,
  ...
  const char *timing = "EAGER";
#else
  const char *timing = "LAZY";
#endif
  arts_printf("=== mrnew_eager_vs_lazy_divergence (%s) ===\n", timing);
  ...
  arts_printf("PASS mrnew_eager_vs_lazy_divergence (%s) %u steps\n", timing,
              STEPS);
  arts_shutdown();
}
