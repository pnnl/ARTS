...
/// @file rcu_baton_invalidate_recheck.c
...
///
/// RCU-only, both timings (the baton + request handler are shared
...
int main(void) {
  printf("SKIP rcu_baton_invalidate_recheck: RCU-only\n");
  return 0;
}
...
static void check_edt(uint32_t paramc, const uint64_t *paramv, uint32_t depc,
  ...
  if (d == NULL || d[0] != expect) {
    (void)fprintf(stderr,
                  "FAIL rcu_baton_invalidate_recheck: expected %u got %u\n",
                  expect, d ? d[0] : 0u);
    arts_abort(1);
  }
}
...
void main_edt(uint32_t paramc, const uint64_t *paramv, uint32_t depc,
  ...
  arts_printf("=== rcu_baton_invalidate_recheck ===\n");
  ...
  arts_printf("PASS rcu_baton_invalidate_recheck sum=%u\n", total);
  arts_shutdown();
}
