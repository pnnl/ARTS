 ...
 *
 * rwlock_multiconsumer_queue — RWLOCK-config-specific stress for the home-side
 ...
 *
 * Runs under RCU/RWLOCK — protocols that serialize same-DB RW, so the
 * accumulator deterministically reaches N_WRITERS.  SKIP under WRF_RCU: DB-WRF
...
int main(void) {
  printf("SKIP rwlock_multiconsumer_queue: serialized-RW invariant (acc=="
         "N_WRITERS) is undefined under WRF_RCU (DB-WRF — unordered concurrent RW "
         "writers race)\n");
  return 0;
}
#else
...
static void verify_edt(uint32_t paramc, const uint64_t *paramv, uint32_t depc,
  ...
  printf("rwlock_multiconsumer_queue: %d writers + %d readers, acc=%llu — PASS\n",
...
void main_edt(uint32_t paramc, const uint64_t *paramv, uint32_t depc,
  ...
  arts_printf("=== rwlock_multiconsumer_queue (%d W + %d R) ===\n", N_WRITERS,
              N_READERS);
