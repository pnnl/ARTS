...
/// @file rcu_eager_writeback_ack.c
...
#if !defined(ARTS_PROTOCOL_RCU) || !defined(ARTS_TIMING_EAGER)

int main(void) {
  printf("SKIP rcu_eager_writeback_ack: RCU+EAGER-only\n");
  return 0;
}
...
static void rd_edt(uint32_t paramc, const uint64_t *paramv, uint32_t depc,
  ...
  if (d == NULL || d[0] != expect) {
    (void)fprintf(stderr,
                  "FAIL rcu_eager_writeback_ack: expected 0x%x got 0x%x\n",
                  expect, d ? d[0] : 0u);
    arts_abort(1);
  }
}
...
void main_edt(uint32_t paramc, const uint64_t *paramv, uint32_t depc,
  ...
  arts_printf("=== rcu_eager_writeback_ack ===\n");
  ...
  arts_printf("PASS rcu_eager_writeback_ack %u steps\n", STEPS);
  arts_shutdown();
}
