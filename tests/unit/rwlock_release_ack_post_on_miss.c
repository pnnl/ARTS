...
/// @file rwlock_release_ack_post_on_miss.c
/// @brief T110 — LOCK_RELEASE_ACK cv-guarded post, even on a cache MISS
///        (B018).
///
/// Under the RWLOCK protocol an RW release writes back to the home and blocks on
/// a stack-local sem_t whose address rides the wire `cv` field; the home wakes
/// the releaser via LOCK_RELEASE_ACK by pointer identity.  Unlike the eager
/// WRITEBACK_ACK (which posts unconditionally), the LOCK_RELEASE_ACK path
...
///
/// Black-box driver: a long cross-rank RW ping-pong forces a RWLOCK release +
...
///
/// Config gate: LOCK_RELEASE_ACK exists only under the RWLOCK protocol; every
/// other protocol uses WRITEBACK_ACK (eager/WRF_RCU) or async transfer (lazy).
/// Compile-time self-skip on non-RWLOCK.
...
int main(void) {
  printf("SKIP rwlock_release_ack_post_on_miss: RWLOCK-only\n");
  return 0;
}
#else
...
/// RW holder: bump the per-DB counter (a real write so the RWLOCK release does a
/// non-empty writeback before its LOCK_RELEASE_ACK).
...
void main_edt(uint32_t paramc, const uint64_t *paramv, uint32_t depc,
  ...
  arts_printf("=== rwlock_release_ack_post_on_miss ===\n");
  ...
  for (unsigned int it = 0; it < ITERS; it++) {
    ...
    /* RW ping-pong: each handoff releases the lock (writeback + ACK).  The
  ...
  arts_printf("PASS: rwlock_release_ack_post_on_miss %u iters x %u ranks\n",
              ITERS, nranks);
  arts_shutdown();
}
