 ...
 *
 * T067 — RWLOCK rwlock_compute_next (lock/home.c), the PURE lock-state transition
 *        function over [state_bit | w:31 | r:31].
 *
 * rwlock_compute_next(cur, op, &grant) is a deterministic pure function: given
 ...
 *
 * rwlock_compute_next is non-static (exposed for this test); the file is built
 * standalone by #including lock/home.c.  RWLOCK-only; self-skips elsewhere.
 *
 * Build: -DARTS_PROTOCOL_RWLOCK=1 -DARTS_UNIT_STANDALONE_SHIMS.
 */
...
int main(void) {
  printf(
      "PASS rwlock_compute_next: skipped (EAGER RWLOCK-only; the EAGER lock_state "
      "machine [state_bit|w|r] exists only in the RWLOCK+EAGER build)\n");
  return 0;
}
#else
...
uint64_t rwlock_compute_next(uint64_t cur, int op, uint32_t *out_grant);
...
static void chk(const char *name, uint32_t in_bit, uint32_t in_w, uint32_t in_r,
  ...
  uint64_t next = rwlock_compute_next(cur, op, &grant);
...
int main(void) {
  ...
  printf("PASS rwlock_compute_next: %d truth-table transitions "
...
#include "core/coherence/rwlock/arbiters.c"
#endif
