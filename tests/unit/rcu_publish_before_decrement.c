...
/// @file rcu_publish_before_decrement.c
...
///
/// RCU-only, both timings (the publish-before-decrement is identical in the
...
int main(void) {
  printf("SKIP rcu_publish_before_decrement: RCU-only\n");
  return 0;
}
...
static void check_edt(uint32_t paramc, const uint64_t *paramv, uint32_t depc,
  ...
  if (d == NULL || d[0] != expect) {
    (void)fprintf(stderr,
                  "FAIL rcu_publish_before_decrement: expected %u got %u\n",
                  expect, d ? d[0] : 0u);
    arts_abort(1);
  }
}
...
void main_edt(uint32_t paramc, const uint64_t *paramv, uint32_t depc,
  ...
  arts_printf("=== rcu_publish_before_decrement ===\n");
  ...
  arts_printf("PASS rcu_publish_before_decrement sum=%u\n", total);
  arts_shutdown();
}
