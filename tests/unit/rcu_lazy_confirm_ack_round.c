...
/// @file rcu_lazy_confirm_ack_round.c
...
#if !defined(ARTS_PROTOCOL_RCU) || !defined(ARTS_TIMING_LAZY)

int main(void) {
  printf("SKIP rcu_lazy_confirm_ack_round: RCU+LAZY-only\n");
  return 0;
}
...
static void check_edt(uint32_t paramc, const uint64_t *paramv, uint32_t depc,
  ...
  if (d == NULL || d[0] != expect) {
    (void)fprintf(stderr,
                  "FAIL rcu_lazy_confirm_ack_round: expected %u got %u\n",
                  expect, d ? d[0] : 0u);
    arts_abort(1);
  }
}
...
void main_edt(uint32_t paramc, const uint64_t *paramv, uint32_t depc,
  ...
  arts_printf("=== rcu_lazy_confirm_ack_round ===\n");
  ...
  arts_printf("PASS rcu_lazy_confirm_ack_round sum=%u\n", total);
  arts_shutdown();
}
