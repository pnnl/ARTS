...
/// auto-acquire RW; the home rank is the initial IDLE owner.  Under MRNEW the
...
///
/// MRNEW-only, both timings (the NO_ACQUIRE creator-home init + first-request
/// transition is in arts_handler_db_create / start_ownership_round, shared).
...
int main(void) {
  printf("SKIP mrnew_no_acquire: MRNEW-only\n");
  return 0;
}
...
static void check_edt(uint32_t paramc, const uint64_t *paramv, uint32_t depc,
  ...
  if (d == NULL || d[0] != expect) {
    (void)fprintf(stderr, "FAIL mrnew_no_acquire: expected %u got %u\n", expect,
                  d ? d[0] : 0u);
    arts_abort(1);
  }
}
...
void main_edt(uint32_t paramc, const uint64_t *paramv, uint32_t depc,
  ...
  arts_printf("PASS mrnew_no_acquire value=%u\n", (unsigned int)expect);
  arts_shutdown();
}
