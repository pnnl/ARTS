 ...
 *
 * T104 — WRF_VAL home-rank writer concurrent with non-home writer: no buffer
 ...
 * PUBLISH packet, which buf_install()s at home under a monotonic CAS.  Under
 ...
 *
 * config_specific: WRF_VAL only — the concurrent cross-rank RW with no ownership
...
void main_edt(uint32_t paramc, const uint64_t *paramv, uint32_t depc,
  ...
  (void)depc;
  (void)depv;
  arts_printf("SKIP wrf_val_home_vs_nonhome_writer: WRF_VAL-only\n");
  arts_shutdown();
}
#else
...
static void verifier_edt(uint32_t paramc, const uint64_t *paramv, uint32_t depc,
  ...
  uint8_t *p = (uint8_t *)depv[0].ptr;
  if (p == NULL) {
    arts_printf("wrf_val_home_vs_nonhome_writer: FAIL got NULL buffer\n");
    arts_abort(1);
  }
  uint8_t v = p[0];
  for (size_t i = 1; i < DB_SIZE; i++) {
    if (p[i] != v) {
      arts_printf(
          "wrf_val_home_vs_nonhome_writer: FAIL torn buffer at %zu (%u vs %u)\n",
          i, (unsigned)p[i], (unsigned)v);
      arts_abort(1); /* corruption: a real bug, never a legal DB-WRF outcome */
    }
  }
  if (v != FILL_HOME && v != FILL_REMOTE) {
    arts_printf("wrf_val_home_vs_nonhome_writer: FAIL unexpected value %u\n",
                (unsigned)v);
    arts_abort(1);
  }
}
...
void main_edt(uint32_t paramc, const uint64_t *paramv, uint32_t depc,
  ...
  if (arts_get_total_ranks() < 2) {
    arts_printf("SKIP wrf_val_home_vs_nonhome_writer: requires 2+ ranks\n");
    arts_shutdown();
    return;
  }
  ...
  arts_printf("PASS wrf_val_home_vs_nonhome_writer: %u rounds, no corruption\n",
              ROUNDS);
  arts_shutdown();
}
#endif
