 ...
 *
 * rwlock_samerank_rw_grant_race — RWLOCK-config-specific reproducer for the
 ...
 * Under the RWLOCK protocol the home grants one rank-granular GRANT and the
 ...
 *
 * The test is CORRECT-AND-FAILING by design under RWLOCK while the bug is
...
#include <stdio.h>
int main(void) {
  printf("SKIP rwlock_samerank_rw_grant_race: RWLOCK-only\n");
  return 0;
}
#else
...
static void verify_edt(uint32_t paramc, const uint64_t *paramv, uint32_t depc,
  ...
  printf("rwlock_samerank_rw_grant_race: %d iters x %d RW = %d grants — PASS\n",
...
void main_edt(uint32_t paramc, const uint64_t *paramv, uint32_t depc,
  ...
  arts_printf("=== rwlock_samerank_rw_grant_race (%d iters x %d RW) ===\n",
              M_ITERS, N_CONSUMERS);
