 ...
 *
 * rwlock_request_coalesce — RWLOCK-config-specific test for the request_in_flight
 ...
 *
 * Self-skips on any non-RWLOCK build at compile time (the coalesce bits are a
 * RWLOCK-cache concept).
 */

#if !defined(ARTS_PROTOCOL_RWLOCK)
#include <stdio.h>
int main(void) {
  printf("SKIP rwlock_request_coalesce: RWLOCK-only\n");
  return 0;
}
#else
...
static void verify_edt(uint32_t paramc, const uint64_t *paramv, uint32_t depc,
  ...
  printf("rwlock_request_coalesce: %d RW + %d RO over %d iters — PASS\n",
...
void main_edt(uint32_t paramc, const uint64_t *paramv, uint32_t depc,
  ...
  arts_printf(
      "=== rwlock_request_coalesce (%d RW + %d RO / iter, %d iters) ===\n", N_RW,
      N_RO, M_ITERS);
