 ...
 * thread counts must fold sender_threads + receiver_threads back into the
 * worker pool (a single node has no network threads).  Per
 * config_compute_derived's `table_length <= 1` branch:
 *
 *     worker += sender + receiver;  sender = 0;  receiver = 0;
 *     thread_count = worker + sender + receiver;
 *
 * With a cfg of worker=2 / sender=3 / receiver=4 on 1 node we therefore expect
 *     worker_thread_count == 9
...
int main(void) {
  ...
  if (config.worker_thread_count != 9) {
    printf("FAIL config_load_single_reclaim: worker=%u expected 9\n",
           config.worker_thread_count);
    ok = 0;
  }
  if (config.sender_thread_count != 0) {
    printf("FAIL config_load_single_reclaim: sender=%u expected 0\n",
           config.sender_thread_count);
    ok = 0;
  }
  if (config.receiver_thread_count != 0) {
    printf("FAIL config_load_single_reclaim: receiver=%u expected 0\n",
           config.receiver_thread_count);
    ok = 0;
  }
  if (config.thread_count != 9) {
    printf("FAIL config_load_single_reclaim: thread_count=%u expected 9\n",
           config.thread_count);
    ok = 0;
  }
  ...
  if (ok) {
    printf("PASS config_load_single_reclaim worker=9 sender=0 receiver=0 "
           "thread_count=9\n");
    return 0;
  }
  return 1;
