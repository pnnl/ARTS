...
/// @file lock_release_ack_post_on_miss.c
/// @brief T110 — LOCK_RELEASE_ACK cv-guarded post, even on a cache MISS
///        (B018).
///
/// Under the LOCK protocol an RW release writes back to the home and blocks on
...
///
/// Black-box driver: a long cross-rank RW ping-pong forces a LOCK release +
...
///
/// Config gate: LOCK_RELEASE_ACK exists only under the LOCK protocol; every
/// other protocol uses WRITEBACK_ACK (eager/MRMW) or async transfer (lazy).
/// Compile-time self-skip on non-LOCK.
...
int main(void) {
  printf("SKIP lock_release_ack_post_on_miss: LOCK-only\n");
  return 0;
}
#else
...
/// RW holder: bump the per-DB counter (a real write so the LOCK release does a
...
void main_edt(uint32_t paramc, const uint64_t *paramv, uint32_t depc,
  ...
  arts_printf("=== lock_release_ack_post_on_miss ===\n");
  ...
  arts_printf("PASS: lock_release_ack_post_on_miss %u iters x %u ranks\n",
              ITERS, nranks);
  arts_shutdown();
}
