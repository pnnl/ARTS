...
///
/// RCU-only, runs under both timings (the contrast is the whole point — the
...
int main(void) {
  printf("SKIP rcu_snapshot_request: RCU-only\n");
  return 0;
}
...
static void rd_edt(uint32_t paramc, const uint64_t *paramv, uint32_t depc,
  ...
  if (d == NULL || d[0] != expect) {
    (void)fprintf(stderr,
                  "FAIL rcu_snapshot_request: expected 0x%x got 0x%x\n",
                  expect, d ? d[0] : 0u);
    arts_abort(1);
  }
}
...
void main_edt(uint32_t paramc, const uint64_t *paramv, uint32_t depc,
...
#endif
  arts_printf("=== rcu_snapshot_request (%s) ===\n", timing);
  ...
  for (unsigned int r = 0; r < ROUNDS; r++) {
     ...
     * hit the dedup/watermark branch (EAGER update_cached_version_max /
  ...
  arts_printf("PASS rcu_snapshot_request (%s) rounds=%u\n", timing, ROUNDS);
  arts_shutdown();
}
