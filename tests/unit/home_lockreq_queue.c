...
static int part1_functional(void) {
  ...
  if (arts_home_lockreq_queue_peek(&q, &r)) {
    (void)fprintf(stderr, "FAIL: peek on empty returned true\n");
    rc = 1;
  }
  if (arts_home_lockreq_queue_pop(&q, &r)) {
  ...
  for (unsigned int i = 0; i < 10; i++) {
    arts_home_lockreq_queue_push(&q, i);
  }
  if (arts_home_lockreq_queue_empty(&q)) {
  ...
  if (!arts_home_lockreq_queue_peek(&q, &p1) ||
      !arts_home_lockreq_queue_peek(&q, &p2) || p1 != 0 || p2 != 0) {
    (void)fprintf(stderr,
  ...
  for (unsigned int i = 0; i < 10; i++) {
    ...
    if (!arts_home_lockreq_queue_pop(&q, &got)) {
...
static int part2_stub_reuse(void) {
  ...
  for (int round = 0; round < 3; round++) {
    for (unsigned int i = 0; i < 5; i++) {
      arts_home_lockreq_queue_push(&q, round * 100 + i);
    }
    for (unsigned int i = 0; i < 5; i++) {
      unsigned int got = 0;
      if (!arts_home_lockreq_queue_pop(&q, &got) ||
...
static void *producer(void *arg) {
  ...
  for (unsigned int s = 0; s < PER_PROD; s++) {
    arts_home_lockreq_queue_push(&g_q, prod * PER_PROD + s);
...
static int part3_mpsc(void) {
  ...
  while (consumed < total) {
    unsigned int v;
    if (!arts_home_lockreq_queue_pop(&g_q, &v)) {
