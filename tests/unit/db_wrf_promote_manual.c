 ...
 *
 * T107 — DB-WRF manual-diagnostic promotion: a CI-safe, deterministically
 *        asserting guard for the two manual lost-update diagnostics
 *        (db_wrf_multi_writer_concurrent, db_wrf_counter_race).
 *
 * Those two diagnostics document the DB-WRF "last-writer-wins" / lost-update
 ...
 * outcomes the DB-WRF contract PERMITS, and never outside it.  This test
 ...
 *   (A) multi-writer (from db_wrf_multi_writer_concurrent): two ranks each fill
 ...
 *   (B) counter race (from db_wrf_counter_race): two ranks each do N untracked
...
/* Final verifier: checks BOTH sub-scenarios' results against the DB-WRF
...
static void verifier_edt(uint32_t paramc, const uint64_t *paramv, uint32_t depc,
  ...
  if (mw == NULL) {
    arts_printf("db_wrf_promote_manual: FAIL (A) NULL buffer\n");
    arts_abort(1);
  }
  uint8_t v = mw[0];
  for (size_t i = 1; i < DB_SIZE; i++) {
    if (mw[i] != v) {
      arts_printf("db_wrf_promote_manual: FAIL (A) torn buffer @%zu\n", i);
      arts_abort(1);
    }
  }
  if (v != 0 && v != 1) {
    arts_printf("db_wrf_promote_manual: FAIL (A) value=%u outside {0,1}\n",
                (unsigned)v);
    arts_abort(1);
  }
  ...
  if (got != n && got != 2u * n) {
    arts_printf("db_wrf_promote_manual: FAIL (B) counter=%llu outside {%llu, "
                "%llu}\n",
  ...
  arts_printf("db_wrf_promote_manual: PASS (A) final=%u (B) counter=%llu "
              "(both in DB-WRF legal set)\n",
...
void main_edt(uint32_t paramc, const uint64_t *paramv, uint32_t depc,
  ...
  if (arts_get_total_ranks() < 2) {
    arts_printf("SKIP db_wrf_promote_manual: requires 2+ ranks\n");
    arts_shutdown();
    return;
  }
