...
/// per DB.
///
/// Correctness: each worker calls arts_abort(1) on NULL ptr; the finish scope
/// draining to shutdown_edt is proof all workers completed without abort.
/// A stranded waiter surfaces as a ctest TIMEOUT.  No global cross-EDT state.
///
/// Spec section 6 B.1.
...
static void worker_edt(uint32_t paramc, const uint64_t *paramv, uint32_t depc,
   ...
   * Either way, just touch the payload to exercise the coherence path. */
...
static void shutdown_edt(uint32_t paramc, const uint64_t *paramv, uint32_t depc,
  ...
  (void)depc;
  (void)depv;
  arts_printf("PASS: %d EDTs completed across %d iterations\n",
              K_ITERS * N_EDTS, K_ITERS);
  arts_shutdown();
}
