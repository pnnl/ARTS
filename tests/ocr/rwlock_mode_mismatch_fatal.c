 ...
 *
 * rwlock_mode_mismatch_fatal — cross-protocol binary mismatch detection driver
 * for the RWLOCK protocol (census 11-lock; dispatcher.c RWLOCK fatal arms at
 * lines ~224/255/288/373/393/531 — snapshot / ownership / writeback wire
 * messages are rejected under RWLOCK, and RWLOCK's own REQUEST/GRANT/RELEASE wire
 * is unknown to non-RWLOCK builds → the receiver's default arm stops the
 * runtime).
 *
 * Like coherence_mode_mismatch.c, this is a STANDALONE driver (not a plain
 * ctest): run_mode_mismatch.sh launches a RWLOCK-built binary as one rank and a
 * non-RWLOCK-built binary (RCU eager/lazy, WRF_RCU) as the other.  Across
 * the protocol boundary the wire tags diverge:
 *   - The non-RWLOCK rank receives MSG_DB_LOCK_REQUEST / _GRANT / _RELEASE
 *     (absent from its enum dispatch) → hits the dispatcher default arm →
 *     arts_shutdown + arts_runtime_stop → non-zero exit.
 *   - The RWLOCK rank receives MSG_DB_SNAPSHOT_* / _OWNERSHIP_* / _WRITEBACK*
 ...
 * RW-ownership/writeback/LOCK_RELEASE divergence, rank 0 creates a DB and wires
...
/* Remote RW writer (rank 1): forces the build's RW-ownership/writeback (RCU)
 * or LOCK_REQUEST(RW)/LOCK_RELEASE (RWLOCK) wire across the rank boundary. */
...
/* Remote RO reader (rank 1): forces the build's RO-snapshot (RCU) or
 * LOCK_REQUEST(RO) (RWLOCK) wire across the rank boundary, then shuts down. */
...
void main_edt(uint32_t paramc, const uint64_t *paramv, uint32_t depc,
  ...
  if (nranks < 2) {
    ...
    arts_printf("SKIP rwlock_mode_mismatch_fatal: requires 2+ ranks (got %u)\n",
                nranks);
    arts_shutdown();
    return;
  }
  ...
  /* Remote RW writer on rank 1: RW path (ownership/writeback vs RWLOCK rel). */
  arts_edt_hint_t wh = ARTS_EDT_HINT_DEFAULTS;
  wh.rank = 1;
  arts_guid_t w = arts_edt_create(writer_edt, 0, NULL, 1, &wh);
  arts_add_dependence(db, w, 0, DB_MODE_RW);

  /* Remote RO reader on rank 1: RO path (snapshot vs RWLOCK req RO). */
