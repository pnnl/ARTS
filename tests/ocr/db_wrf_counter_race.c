...
static void verifier_edt(uint32_t paramc, const uint64_t *paramv, uint32_t depc,
  ...
  if (v == expected) {
    arts_printf("DB_WRF_COUNTER_RACE: counter=%lu (expected=%lu) — "
                "DETERMINISTIC (OCR-like)\n",
                v, expected);
  } else if (v == INCREMENTS_PER_RANK) {
    arts_printf("DB_WRF_COUNTER_RACE: counter=%lu (expected=%lu) — RACED "
                "(DB-WRF-like, lost-update)\n",
                v, expected);
  } else {
    arts_printf("DB_WRF_COUNTER_RACE: counter=%lu (expected=%lu) — "
                "UNEXPECTED\n",
...
void main_edt(uint32_t paramc, const uint64_t *paramv, uint32_t depc,
  ...
  if (arts_get_total_ranks() < 2) {
    arts_printf("DB_WRF_COUNTER_RACE: SKIP requires 2+ ranks\n");
    arts_shutdown();
    return;
  }
