...
void main_edt(uint32_t paramc, const uint64_t *paramv, uint32_t depc,
   ...
   * acquire until the writer releases (writes back).  MRMW is DB-DRF — the RW
   ...
   * home before the writeback publishes and observe a stale value.  Not a
   * defined program under DB-DRF. */
  arts_printf(
      "SKIP lock_req_before_create: relies on RW serialization (MRMW is "
      "DB-DRF)\n");
  arts_shutdown();
  return;
#endif
