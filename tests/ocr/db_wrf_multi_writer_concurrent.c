...
/// @file db_wrf_multi_writer_concurrent.c
/// @brief DB-WRF multi-writer test: two ranks each acquire RW and
///        write their rank-id into a shared DB.  Under the DB-WRF
...
static void verifier_edt(uint32_t paramc, const uint64_t *paramv, uint32_t depc,
  ...
  /* Under the DB-WRF contract the final value is one of the writer rank-ids. */
  if (consistent && (v == 0 || v == 1)) {
    arts_printf("DB_WRF_MULTI_WRITER: PASS final=%u\n", (unsigned)v);
  } else {
    arts_printf("DB_WRF_MULTI_WRITER: FAIL value=%u consistent=%d\n",
...
void main_edt(uint32_t paramc, const uint64_t *paramv, uint32_t depc,
  ...
  if (arts_get_total_ranks() < 2) {
    arts_printf("DB_WRF_MULTI_WRITER: SKIP requires 2 ranks\n");
    arts_shutdown();
    return;
  }
