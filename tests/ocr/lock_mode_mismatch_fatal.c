 ...
 *
 * lock_mode_mismatch_fatal — cross-protocol binary mismatch detection driver
 * for the LOCK protocol (census 11-lock; dispatcher.c LOCK fatal arms at
 * lines ~224/255/288/373/393/531 — snapshot / ownership / writeback wire
 * messages are rejected under LOCK, and LOCK's own REQUEST/GRANT/RELEASE wire
 * is unknown to non-LOCK builds → the receiver's default arm stops the
 * runtime).
 *
 * Like coherence_mode_mismatch.c, this is a STANDALONE driver (not a plain
 * ctest): run_mode_mismatch.sh launches a LOCK-built binary as one rank and a
 * non-LOCK-built binary (MRNEW eager/lazy, MRSW, MRMW) as the other.  Across
 * the protocol boundary the wire tags diverge:
 *   - The non-LOCK rank receives MSG_DB_LOCK_REQUEST / _GRANT / _RELEASE
 *     (absent from its enum dispatch) → hits the dispatcher default arm →
 *     arts_shutdown + arts_runtime_stop → non-zero exit.
 *   - The LOCK rank receives MSG_DB_SNAPSHOT_* / _OWNERSHIP_* / _WRITEBACK*
...
/* Remote RW writer (rank 1): forces the build's RW-ownership/writeback (MRNEW)
 * or LOCK_REQUEST(RW)/LOCK_RELEASE (LOCK) wire across the rank boundary. */
...
/* Remote RO reader (rank 1): forces the build's RO-snapshot (MRNEW) or
 * LOCK_REQUEST(RO) (LOCK) wire across the rank boundary, then shuts down. */
...
void main_edt(uint32_t paramc, const uint64_t *paramv, uint32_t depc,
  ...
  if (nranks < 2) {
    ...
    arts_printf("SKIP lock_mode_mismatch_fatal: requires 2+ ranks (got %u)\n",
                nranks);
    arts_shutdown();
    return;
  }
  ...
  /* Remote RW writer on rank 1: RW path (ownership/writeback vs LOCK rel). */
  arts_edt_hint_t wh = ARTS_EDT_HINT_DEFAULTS;
  wh.rank = 1;
  arts_guid_t w = arts_edt_create(writer_edt, 0, NULL, 1, &wh);
  arts_add_dependence(db, w, 0, DB_MODE_RW);

  /* Remote RO reader on rank 1: RO path (snapshot vs LOCK req RO). */
