...
/// never crashed — ctest would mis-report PASS.
...
static void shutdown_edt(uint32_t paramc, const uint64_t *paramv, uint32_t depc,
   ...
   * EDT never fires (the finish scope never completes); rank 0 will instead
   * exit arts_rt() through the peer-disconnect SHUTDOWN_MSG path with the flag
   * still 0, and main() will return non-zero so ctest sees FAIL. */
...
int main(int argc, char **argv) {
  arts_rt(argc, argv);
  /* Rank 0 reaches here when arts_rt() returns.  If shutdown_edt fired,
   * the flag is set; otherwise rank 0 was force-shut by a peer
   * disconnect (a consumer aborted) and the test must report FAIL.
   * Other ranks (1, 2, 3) are forked children — they reach here too,
   * but the SKIP / abort message has already been emitted on the
