...
void main_edt(uint32_t paramc, const uint64_t *paramv, uint32_t depc,
  ...
  for (unsigned int i = 0; i < num_writes; i++) {
    ...
    arts_edt_create(write_test, 3, args, 2,
                    &(arts_edt_hint_t){.guid = write_guids[i]});
    arts_add_dependence(db_guid, write_guids[i], 0, DB_MODE_RW);
  }

  /* write_guids[0] is initialized by the loop above; the analyzer flags it only
   * on the num_writes==0 branch, which this chain test never exercises (chain
   * length is always >= 1). */
