...
void successor(uint32_t paramc, const uint64_t *paramv, uint32_t depc,
  ...
  (void)depc;
  (void)depv;
  int val = atomic_load_explicit(&g_done, memory_order_acquire);
  if (val != 8) {
    arts_printf("FAIL continuation-style: g_done=%d (expected 8)\n", val);
    arts_shutdown();
    return;
  }
...
void orchestrator(uint32_t paramc, const uint64_t *paramv, uint32_t depc,
  ...
  int val = atomic_load_explicit(&g_done, memory_order_acquire);
  if (val != 4) {
    arts_printf("FAIL wait-style: g_done=%d (expected 4)\n", val);
    arts_shutdown();
    return;
  }
