...
void start_iter(uint32_t paramc, const uint64_t *paramv,
                uint32_t depc, arts_edt_dep_t depv[]);

void recursive_spawn(uint32_t paramc, const uint64_t *paramv,
                     uint32_t depc, arts_edt_dep_t depv[]) {
...
void iter_term(uint32_t paramc, const uint64_t *paramv,
               uint32_t depc, arts_edt_dep_t depv[]) {
  unsigned int iter = arts_atomic_add(&iters_done, 1U); /* returns new value, 1..ITERATIONS */
  if ((iter % 10) == 0) {
