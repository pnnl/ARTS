...
void fork_nqueens(uint32_t paramc, const uint64_t *paramv, uint32_t depc,
  ...
  srand(time(NULL) + arts_get_current_node());
  unsigned int route = rand() % num_nodes;
  arts_guid_t join_guid = arts_edt_create(join_nqueens, route, 2, paramv, count);
  for (int i = 0; i < count; i++) {
    ...
    arts_guid_t fork_guid =
        arts_edt_create(fork_nqueens, rand() % num_nodes, 2, new_paramv, 1);
...
void arts_main(int argc, char **argv) {
  ...
  unsigned int route = rand() % num_nodes;
  arts_guid_t final_guid = arts_edt_create(final_nqueens, route, 2, final_paramv, 1);
  uint64_t fork_paramv[2] = {final_guid, 0};
  route = rand() % num_nodes;
  arts_guid_t fork_guid = arts_edt_create(fork_nqueens, route, 2, fork_paramv, 1);
