...
int solve_sequential(int board[], int row, int n) {
  if (row == n) {
    return 1;
  }
  ...
  for (int col = 0; col < n; col++) {
    if (is_safe(board, row, col)) {
      board[row] = col;
      count += solve_sequential(board, row + 1, n);
    }
  }
  return count;
}
...
int solve_parallel_for_dynamic(int n) {
...
#pragma omp parallel for reduction(+ : total_solutions) schedule(dynamic)
  for (int first_col = 0; first_col < n; first_col++) {
    ...
    int local_count = solve_sequential(board, 1, n);
...
int solve_task_based_fixed(int n) {
  int total_solutions = 0;
...
int solve_memory_optimized(int n) {
  int total_solutions = 0;
...
#pragma omp for schedule(dynamic)
    for (int first_col = 0; first_col < n; first_col++) {
      ...
      int local_count = solve_sequential(thread_board, 1, n);
      total_solutions += local_count;
    }
...
int solve_nested_parallel(int n) {
  int total_solutions = 0;
...
#pragma omp parallel for reduction(+ : first_level_count) schedule(dynamic)    \
    ...
    for (int second_col = 0; second_col < n; second_col++) {
      ...
      int local_count = solve_sequential(board, 2, n);
...
int main(int argc, char *argv[]) {
  ...
  solutions = solve_parallel_for_dynamic(n);
  ...
  start_time = omp_get_wtime();
  solutions = solve_task_based_fixed(n);
  end_time = omp_get_wtime();
  printf("   Solutions: %d, Time: %.4f seconds\n", solutions,
         end_time - start_time);
  ...
  start_time = omp_get_wtime();
  solutions = solve_memory_optimized(n);
  end_time = omp_get_wtime();
  printf("   Solutions: %d, Time: %.4f seconds\n", solutions,
         end_time - start_time);
  ...
  start_time = omp_get_wtime();
  solutions = solve_work_stealing(n);
  end_time = omp_get_wtime();
  printf("   Solutions: %d, Time: %.4f seconds\n", solutions,
         end_time - start_time);
  ...
  if (n >= 10) {
    ...
    start_time = omp_get_wtime();
    solutions = solve_nested_parallel(n);
    end_time = omp_get_wtime();
    printf("   Solutions: %d, Time: %.4f seconds\n", solutions,
           end_time - start_time);
  }
