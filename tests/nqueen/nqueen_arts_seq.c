...
void solve_parallel(int board[], int row, int n, int *local_count,
                   int *task_count) {
  if (row == n) {
    (*local_count)++;
    return;
  ...
  for (int col = 0; col < n; col++) {
    if (is_safe(board, row, col)) {
      ...
      solve_parallel(new_board, row + 1, n, local_count, task_count);
...
int solve_nqueens(int n, int *total_tasks) {
  ...
  solve_parallel(board, 0, n, &solution_count, &task_count);

  free(board);
  return solution_count;
}

int arts_main(int argc, char **argv) {
  ...
  int total_tasks = 0;
  int solutions = solve_nqueens(n, &total_tasks);
