...
///            rank). Exercises the per-EDT RW pipelining (multiple
///            OWNERSHIP_REQUEST/PROCEED in flight) plus parallel RO reads.
...
static atomic_int g_check_result = 0; /* 1=pass, -1=wrong, 0=never ran */
...
static void *wd_thread(void *arg) {
  ...
  (void)fprintf(stderr,
                "HANG: coherence_rw_pipeline did not complete in 25s\n");
  (void)fflush(stderr);
  _exit(1);
}
...
int main(int argc, char **argv) {
  ...
  if (arts_get_current_rank() == 0 && atomic_load(&g_check_result) != 1) {
    (void)fprintf(stderr, "FAIL: coherence_rw_pipeline result=%d\n",
