 ...
 *
 * Observed behavior (both OCR-model and MRMW/DB-DRF builds):
 ...
 * The DB-DRF (MRMW) model has no LOCK_REQ chain by design and shows
 * the same lost-update pattern.
 *
 * For a clean OCR/DB-DRF differentiation see CTest:
 * coherence_stress_dist and coherence_lock_req_before_create both PASS
 * under the OCR model and FAIL under the MRMW model because they
 * exercise patterns that the DB-DRF contract does not implement.
 */
...
static void verifier_edt(uint32_t paramc, const uint64_t *paramv, uint32_t depc,
  ...
  if (v == expected) {
    arts_printf("DB_DRF_COUNTER_RACE: counter=%lu (expected=%lu) — "
                "DETERMINISTIC (OCR-like)\n",
                v, expected);
  } else if (v == INCREMENTS_PER_RANK) {
    arts_printf("DB_DRF_COUNTER_RACE: counter=%lu (expected=%lu) — RACED "
                "(DB-DRF-like, lost-update)\n",
                v, expected);
  } else {
    arts_printf("DB_DRF_COUNTER_RACE: counter=%lu (expected=%lu) — "
                "UNEXPECTED\n",
...
void main_edt(uint32_t paramc, const uint64_t *paramv, uint32_t depc,
  ...
  if (arts_get_total_ranks() < 2) {
    arts_printf("DB_DRF_COUNTER_RACE: SKIP requires 2+ ranks\n");
    arts_shutdown();
    return;
  }
