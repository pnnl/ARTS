...
arts_guid_t localDbCreate(void **addr, uint64_t size, arts_type_t mode,
                         arts_guid_t guid) {
  unsigned int db_size = size + sizeof(struct arts_db);
  //    void * ptr = arts_malloc(db_size);
  void *ptr = arts_cuda_malloc_host(db_size);
  if (ptr) {
    struct arts_header *header = (struct arts_header *)ptr;
    header->type = mode;
    header->size = db_size;
...
int main(void) {
  ...
  // {
  //     paramv[i] = i;
  //     depv[i].guid = localDbCreate(&depv[i].ptr, sizeof(arts_guid_t),
