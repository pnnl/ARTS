...
void arts_handler_db_writeback(void *item_v, void *args_v) {
   ...
   * an A(v)-vs-B(v+1) writeback reorder over distinct connections cannot
   * clobber newer home data with older. */
...
void arts_handler_db_ownership_response(void *payload, size_t size) {
  ...
  /* Pin the db_s for the whole handler body (buffer install + pending_rw drain
   * + writer_count RMW): the embedded cache is its FIRST member (offset 0), so
   * one cb ref keeps it alive against a concurrent DESTROY on another receiver
   * thread.  Released on every return path. */
