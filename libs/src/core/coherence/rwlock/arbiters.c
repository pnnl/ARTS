 ...
 *
 * RWLOCK protocol pure state-transition arbiters (EAGER timing only).
 *
 * Defines: rwlock_compute_next, cache_compute_next.
 *
 * These arbiters use the EAGER single-word layouts (lock_state with
 * [state_bit|w|r] and cache_state with [rw_st|ro_st|wc|rc]).  They are
 * EAGER-only and compiled only when ARTS_TIMING_EAGER is defined.  The LAZY
 * arbiters (lock_lazy_compute_next, cache_lazy_compute_next) use a different
 * word layout and live in lazy.c.
 *
 * eager.c #includes this file to get its private copy of the arbiters.
 * The whitebox unit test (tests/unit/rwlock_compute_next.c) also #includes
...
/* ===== cache_compute_next ==============================================
 * Cache-side analogue of rwlock_compute_next: pure function of the current
