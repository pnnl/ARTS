 ...
 *
 * RWLOCK protocol LAZY timing translation unit.
 *
 ...
 *
 * Compiled only for ARTS_COHERENCE_PROTOCOL=RWLOCK + ARTS_PROTOCOL_TIMING=LAZY.
 ...
 * prototypes, and arts_db_lock_waiter_s for the RWLOCK build. */
  ...
  case CACHE_OP_REL_RW: /* precond: rws==GRANT, own==1 */
    ...
    if (wc == 0u && rc == 0u) {
      ...
      if (mt != ARTS_LOCK_NO_TARGET) {
         ...
         * RWLOCK-LAZY analogue of RCU's 0-edge transfer, but a single CAS — no
...
/* ===== arts_db_cache_init ==============================================
 * Initialize the per-rank cache for the RWLOCK-LAZY protocol.
 *
 ...
 * depv[slot].mode.
 *
 * Two-CAS acquire (the EAGER RWLOCK discipline, LAZY arbiter):
