 ...
 *
 * RWLOCK protocol EAGER timing translation unit.
 *
 * Defines: rwlock_compute_next, cache_compute_next,
 ...
 *
 * Compiled only for ARTS_COHERENCE_PROTOCOL=RWLOCK + ARTS_PROTOCOL_TIMING=EAGER.
 ...
 * arts_home_lockreq_queue_s for the RWLOCK build (coherence.h and handlers.h
 ...
 * rwlock_compute_next and cache_compute_next are pure functions (no external
 * calls, no global state).  They live in arbiters.c so the unit test
 * (tests/unit/rwlock_compute_next.c) can #include just the arbiters without
...
void arts_send_db_lock_grant(unsigned int requester_rank, arts_guid_t db_guid,
  ...
  if (requester_rank == arts_global_rank_id) {
    ...
    arts_handler_db_lock_grant(&p, sizeof(p));
    return;
  }
...
void arts_handler_db_lock_request(void *item_v, void *args_v) {
  ...
  do {
    ...
    next = rwlock_compute_next(cur, op, &grant);
...
void arts_send_db_lock_release(unsigned int home_rank, arts_guid_t db_guid,
  ...
  if (home_rank == arts_global_rank_id) {
     ...
     * dispatcher does — HIT runs arts_handler_db_lock_release inline (which
    ...
    if (data == NULL || ds == 0u) {
      ...
      arts_ooo_dispatch_or_defer_guid(db_guid, OOO_DB_LOCK_RELEASE, &args,
                                      sizeof(args));
      return;
    }
    uint32_t asz =
