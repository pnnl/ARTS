 ...
 *
 * This TU contains only the shared pieces that both the EAGER and LAZY timing
 ...
 * specific arbiters (rwlock_compute_next, cache_compute_next) and all
 * acquire/release handler/sender bodies live in the per-timing TU (eager.c
 * or lazy.c).
 *
 ...
 * arts_home_lockreq_queue_s for the RWLOCK build (home.h declares functions
...
void arts_db_home_init(struct arts_db_s *db, unsigned int rw_holder,
  ...
  (void)rw_holder; /* EAGER tracks mode via lock_state phase bit, not a
                    * separate rw_holder field */
  atomic_store_explicit(&db->lock_state, LOCK_MAKE_STATE(0u, 1u, 0u),
                        memory_order_relaxed);
#else /* ARTS_TIMING_LAZY */
  /* LAZY: the home is a pure directory.  The creator is the first DATA owner
   ...
   * FORWARD routes to.  (Mirrors the model checker World init lock_state =
   * (IDLE, HOME, 0, 0) with is_owner[HOME]=true.)  This differs from EAGER,
   * where the creator's RELEASE drives a seeded w=1 back to 0. */
