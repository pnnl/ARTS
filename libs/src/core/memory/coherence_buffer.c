...
static void arts_coherence_buffer_deleter(void *obj) { arts_free(obj); }
...
arts_coherence_install_buffer(struct arts_db_cache_s *cache,
                              ...
                              uint64_t db_size) {
  struct arts_db_buffer_s *new_buf = arts_coherence_buffer_alloc(db_size);
  if (new_buf == NULL) {
  ...
  arts_shared_ptr_t new_cb =
      arts_shared_make(new_buf, arts_coherence_buffer_deleter);
  new_buf->cb = new_cb;
