 ...
 *   - OWNERSHIP_REQUEST: RC/LRC only (release.c); LC's enum omits it.
 *   - WRITEBACK: RC/LC only; LRC's enum omits it (LRC fatals on the wire).
 *   - OWNERSHIP_INVALIDATE: RC only.  RC can see a GRANT/INVALIDATE reorder (or
 *     a before-create race) that lands INVALIDATE before the cache installs, so
 *     it defers + replays here.  LRC never defers INVALIDATE (home publishes
 * the rw_holder target only after that rank's cache install, so the
 *     dispatcher/self-send call the body directly) and LC has no ownership
 *     transfer, so neither model's enum carries this kind. */
...
void arts_ooo_dispatch_or_defer(struct arts_route_item_s *slot,
  ...
  if (h) {
     ...
     * drop.  RC-only: that kind exists solely in the RC build's enum (LRC/LC
     * never defer INVALIDATE). */
#if defined(ARTS_MEMORY_MODEL_RC)
