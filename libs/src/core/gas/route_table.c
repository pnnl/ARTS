...
arts_route_table_t *arts_new_route_table(unsigned int route_table_size,
  ...
  for (uint64_t i = 0; i < total_slots; i++) {
    arts_lf_stack_init(&route_table->data[i].oooList);
  }
  return route_table;
}
...
/* Idempotent install: wrap obj in a cb (deleter by kind) and CAS it into an
 * empty slot.  If the slot is already occupied, return the existing object
 * (obj is NOT wrapped). */
void *arts_route_table_add_item(void *obj, arts_guid_t key, unsigned int rank,
  ...
  arts_shared_ptr_t cur = arts_atomic_shared_load(&item->value);
  if (cur) {
    void *existing = arts_shared_get(cur);
    arts_shared_release(&cur);
    return existing;
  }
  ...
  /* Lost the install race — abandon our cb (object stays the caller's) and
   * return the winner's object. */
  arts_shared_abandon(&cb);
  arts_shared_ptr_t w = arts_atomic_shared_load(&item->value);
  void *existing = w ? arts_shared_get(w) : NULL;
  if (w) {
    arts_shared_release(&w);
  }
  return existing;
}
