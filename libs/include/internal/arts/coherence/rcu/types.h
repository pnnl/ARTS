 ...
 *
 * Selected by arts/coherence/types.h when ARTS_PROTOCOL_WRF_RCU is NOT defined.
...
struct arts_db_cache_s {
   ...
   * via GET_DATA with the home->cached_version watermark, so the eager
   * owner→owner transfer ships an EMPTY map.  The field exists so the shared
   * arts_db_send_ownership_response (coherence/rcu/ownership.c) compiles for
