 ...
 *
 * Selected by arts/coherence/types.h when ARTS_PROTOCOL_RWLOCK is defined.
 ...
 * (owner-bit set) does.  home.cache.buffer is unused by the RWLOCK-LAZY path. */
 ...
 * rwlock_compute_next(cur, op, &grant) is the home arbiter, a pure function of
 ...
 * Same CAS-retry pattern as EAGER's rwlock_compute_next. */
#ifdef ARTS_TIMING_LAZY
#define LOCK_OP_RW_ACQ 0
#define LOCK_OP_RO_ACQ 1
 ...
 * rwlock_compute_next: a pure function of the current word, run inside a
...
#define CACHE_ACT_REL_RW 7     /* send RW RELEASE (writeback, ACK-gated) */
#define CACHE_ACT_REL_RO 8     /* send RO RELEASE (notify) */

/* Defined in arbiters.c (included by eager.c); exposed non-static for the
 * cache-state model test (mirrors rwlock_compute_next's exposure). */
