...
/**
 * @file wrf_rcu/types.h
 * @brief WRF_RCU (Multi-Reader, Multi-Writer / DB-WRF) cache/db layout.
 *
 * Selected by arts/coherence/types.h when ARTS_PROTOCOL_WRF_RCU is defined.
 * WRF_RCU carries no ownership lease and no RW waiter queue: writer_count is a
...
struct arts_db_cache_s {
  ...
  arts_guid_t db_guid;
  uint64_t db_size;
  /* WRF_RCU: writer_count is a pure ref count.  The WRITEBACK ACK rendezvous is
