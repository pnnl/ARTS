...
bool arts_event_create_internal(arts_guid_t *guid, unsigned int route,
  ...
  unsigned int event_size =
      sizeof(struct arts_event_s) + (sizeof(struct arts_dependent_s) * dependent_count);
...
void arts_event_destroy(arts_guid_t guid) {
  struct arts_event_s *event = (struct arts_event_s *)arts_route_table_lookup_item(guid);
  if (event != NULL) {
...
struct arts_link_list_s *arts_get_event_versions(struct arts_persistent_event_s *event) {
...
arts_push_persistent_event_version(struct arts_persistent_event_s *event) {
  ...
  if (versions && versions->tailPtr) {
    last =
        (struct arts_persistent_event_version_s *)arts_link_list_get_tail_data(versions);
}
...
bool arts_persistent_event_free_version(struct arts_persistent_event_s *event) {
  ...
  if (last) {
    ...
    struct arts_link_list_item_s *item = ((struct arts_link_list_item_s *)version) - 1;
    arts_free(item);
  }
...
void arts_persistent_event_satisfy(arts_guid_t event_guid, uint32_t action,
  ...
  struct arts_persistent_event_s *event =
      (struct arts_persistent_event_s *)arts_route_table_lookup_item(event_guid);
  if (!event) {
    unsigned int rank = arts_guid_get_rank(event_guid);
    if (rank != arts_global_rank_id) {
