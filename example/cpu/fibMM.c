...
double mm(unsigned int x, unsigned int y) {
  int n = 1024;
  double *A = artsMalloc(n * n * sizeof(double));
  double *B = artsMalloc(n * n * sizeof(double));
  double *C = artsMalloc(n * n * sizeof(double));
...
void fibJoin(uint32_t paramc, uint64_t *paramv, uint32_t depc,
  ...
  artsSignalEdtValue(paramv[0], paramv[1], mm(x, y));
}
...
void fibFork(uint32_t paramc, uint64_t *paramv, uint32_t depc,
  ...
  if (num < 2)
    ...
    args[2] = num - 2;
    artsEdtCreate(fibFork, next, 3, args, 0);
    // artsEdtCreate(fibFork, 1, 3, args, 0);
  }
}
...
void fibDone(uint32_t paramc, uint64_t *paramv, uint32_t depc,
  ...
  PRINTF("Fib MM %u: %u time: %lu nodes: %u workers: %u\n", paramv[0],
