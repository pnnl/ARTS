...
artsGuid_t *aTileGuids = NULL;
artsGuid_t *bTileGuids = NULL;
...
void copyKernel(uint32_t paramc, uint64_t *paramv, uint32_t depc,
                           artsEdtDep_t depv[]) {
  ...
  for (int idx=0; idx<len; idx++)
    b[idx] = a[idx];
  artsCXLProducerFlush(depv[1].guid);
}

void scaleKernel(uint32_t paramc, uint64_t *paramv, uint32_t depc,
                            artsEdtDep_t depv[]) {
  unsigned int len = (unsigned int)paramv[0];
...
void addKernel(uint32_t paramc, uint64_t *paramv, uint32_t depc,
                          artsEdtDep_t depv[]) {
  unsigned int len = (unsigned int)paramv[0];
  double *a = (double *)depv[0].ptr;
  double *b = (double *)depv[1].ptr;
  ...
  for (int idx=0; idx<len; idx++)
    c[idx] = a[idx] + b[idx];
  artsCXLProducerFlush(depv[2].guid);
}

void triadKernal(uint32_t paramc, uint64_t *paramv, uint32_t depc,
                            artsEdtDep_t depv[]) {
  unsigned int len = (unsigned int)paramv[0];
  double scale = (double)paramv[1];
  double *a = (double *)depv[0].ptr;
...
void streamDriver(uint32_t paramc, uint64_t *paramv, uint32_t depc,
                  artsEdtDep_t depv[]) {
  int j, k;
  ...
  for (k = 0; k < NTIMES; k++) {
    ...
    epochGuid = artsInitializeAndStartEpoch(NULL_GUID, 0);
    launch2KernelEdt(copyKernel, tileSize, N, 0, aTileGuids, cTileGuids);
  ...
  if (!artsGetCurrentNode()) {
    epochGuid = artsInitializeAndStartEpoch(NULL_GUID, 0);
    forceAcquireCXLGuids(numTiles, aTileGuids, bTileGuids, cTileGuids);
    artsWaitOnHandle(epochGuid);  
    // for (unsigned int i = 0; i < numTiles; i++) {
    //   artsCXLConsumerFlush(aTileGuids[i]);
    //   artsCXLConsumerFlush(bTileGuids[i]);
    //   artsCXLConsumerFlush(cTileGuids[i]);
    // }
...
void initPerNode(unsigned int nodeId, int argc, char **argv) {
  ...
  aTileGuids = malloc(sizeof(artsGuid_t)*numTiles);
  bTileGuids = malloc(sizeof(artsGuid_t)*numTiles);
  cTileGuids = malloc(sizeof(artsGuid_t)*numTiles);
  ...
  if (!nodeId) {
    ...
    for (unsigned int i = 0; i < numTiles; i++) {
     aTileGuids[i] = artsDbCreate((void **)&(aTile[i]), tileSize * sizeof(double),
                                  ARTS_DB_CXL);
     bTileGuids[i] = artsDbCreate((void **)&(bTile[i]), tileSize * sizeof(double),
                                  ARTS_DB_CXL);
     cTileGuids[i] = artsDbCreate((void **)&(cTile[i]), tileSize * sizeof(double),
                                  ARTS_DB_CXL);
      ...
      artsCXLProducerFlush(aTileGuids[i]);
      artsCXLProducerFlush(bTileGuids[i]);
      artsCXLProducerFlush(cTileGuids[i]);
    }
