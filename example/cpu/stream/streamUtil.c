...
void launch2KernelEdt(artsEdt_t funPtr, unsigned int tileSize,
                      unsigned int totalSize, double scalar,
                      artsGuid_t *aGuid, artsGuid_t *bGuid) {
  ...
  if (scalar != 0) {
    for (unsigned int i = 0; i < tiles; ++i) {
      ...
      artsGuid_t edtGuid =
          artsEdtCreate(funPtr, next, 2, args, 2);
      next = (next + 1) % artsGetTotalNodes();
      /*
      artsSignalEdt(edtGuid, 0, artsGetGuid(aGuid, i));
      artsSignalEdt(edtGuid, 1, artsGetGuid(bGuid, i));
      */
      artsSignalEdt(edtGuid, 0, aGuid[i]);
      artsSignalEdt(edtGuid, 1, bGuid[i]);
    }
  } else {
    for (unsigned int i = 0; i < tiles; ++i) {
      args[0] = (i + 1 < tiles) ? tileSize : totalSize - i * tileSize;
      artsGuid_t edtGuid =
          artsEdtCreate(funPtr, next, 1, args, 2);
      next = (next + 1) % artsGetTotalNodes();
      /*
      artsSignalEdt(edtGuid, 0, artsGetGuid(aGuid, i));
      artsSignalEdt(edtGuid, 1, artsGetGuid(bGuid, i));
...
void launch3KernelEdt(artsEdt_t funPtr, unsigned int tileSize,
  ...
  if (scalar != 0) {
    for (unsigned int i = 0; i < tiles; ++i) {
      args[0] = (i + 1 < tiles) ? tileSize : totalSize - i * tileSize;
      artsGuid_t edtGuid =
          artsEdtCreate(funPtr, next, 2, args, 3);
      next = (next + 1) % artsGetTotalNodes();
      /*
      artsSignalEdt(edtGuid, 0, artsGetGuid(aGuid, i));
      artsSignalEdt(edtGuid, 1, artsGetGuid(bGuid, i));
      ...
      */
      artsSignalEdt(edtGuid, 0, aGuid[i]);
      artsSignalEdt(edtGuid, 1, bGuid[i]);
      artsSignalEdt(edtGuid, 2, cGuid[i]);
    }
  } else {
    for (unsigned int i = 0; i < tiles; ++i) {
      args[0] = (i + 1 < tiles) ? tileSize : totalSize - i * tileSize;
      artsGuid_t edtGuid =
          artsEdtCreate(funPtr, next, 1, args, 3);
      next = (next + 1) % artsGetTotalNodes();
      /*
      artsSignalEdt(edtGuid, 0, artsGetGuid(aGuid, i));
      artsSignalEdt(edtGuid, 1, artsGetGuid(bGuid, i));
...
void doNothing(uint32_t paramc, uint64_t *paramv, uint32_t depc,
                            artsEdtDep_t depv[]) {
}

void forceAcquireCXLGuids(unsigned int numTiles, 
                      artsGuid_t *aGuid, artsGuid_t *bGuid,
                      artsGuid_t *cGuid) {
  for (unsigned int i = 0; i < numTiles; i++) {
    artsGuid_t edtGuid = artsEdtCreate(doNothing, 0, 0, NULL, 3);
    artsSignalEdt(edtGuid, 0, aGuid[i]);
    artsSignalEdt(edtGuid, 1, bGuid[i]);
    artsSignalEdt(edtGuid, 2, cGuid[i]);
  }
