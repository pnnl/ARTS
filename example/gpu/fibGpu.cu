...
__global__ void fibJoin(uint32_t paramc, uint64_t *paramv, uint32_t depc,
                        artsEdtDep_t depv[]) {
...
void fibFork(uint32_t paramc, uint64_t *paramv, uint32_t depc,
  ...
  if ((*resPtr) < 2)
    ...
    unsigned int *x = NULL;
    artsGuid_t xGuid =
        artsDbCreate((void **)&x, sizeof(unsigned int), ARTS_DB_GPU_WRITE);
    (*x) = (*resPtr) - 1;

    unsigned int *y = NULL;
    artsGuid_t yGuid =
        artsDbCreate((void **)&y, sizeof(unsigned int), ARTS_DB_GPU_WRITE);
    (*y) = (*resPtr) - 2;

    // Create a continuation edt to run on the GPU
    dim3 grid(1);
    dim3 block(1);
    artsGuid_t joinGuid = artsEdtCreateGpu(fibJoin, next, 0, NULL, 3, grid,
                                           block, doneGuid, slot, resGuid);
...
extern "C" void initPerWorker(unsigned int nodeId, unsigned int workerId,
  ...
  if (!nodeId && !workerId) {
    ...
    artsGuid_t resGuid =
        artsDbCreate((void **)&resPtr, sizeof(unsigned int), ARTS_DB_GPU_WRITE);
    if (argc < 2) {
