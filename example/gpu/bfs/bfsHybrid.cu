...
void cpuBfs(uint32_t paramc, uint64_t *paramv, uint32_t depc,
  ...
  for (unsigned int index = 0; index < currentFrontierSize; index++) {
    ...
    if (success) {
      ...
      if (neighborCount) {
        unsigned int frontierIndex =
            artsAtomicFetchAdd(&cpuBufferCount, (unsigned int)neighborCount);
        if (frontierIndex < GPULISTLEN) {
          for (uint64_t i = 0; i < neighborCount; ++i) {
...
void cpuSort(uint32_t paramc, uint64_t *paramv, uint32_t depc,
      ...
      GPULISTLEN) // If it was bigger than the search frontier, we need to quit
  {
    ...
    // Remove duplicates
    newFrontierCount =
        std::unique(cpuBuffer, cpuBuffer + newFrontierCount) - cpuBuffer;
    ...
    for (unsigned int i = 0; i < PARTS; i++)
      upperIndexPerBound[i] =
          std::upper_bound(cpuBuffer, cpuBuffer + newFrontierCount, bounds[i]) -
          cpuBuffer;
    ...
    for (unsigned int i = 0; i < PARTS; i++) {
      if (sizePerBound[i]) {
        ...
        *newSearchFrontier = sizePerBound[i];
        ...
        memcpy((void *)(newSearchFrontier + 1), (void *)(cpuBuffer + tempIndex),
