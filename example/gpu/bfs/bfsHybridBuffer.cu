...
void createFirstRound(uint32_t paramc, uint64_t *paramv, uint32_t depc,
                      artsEdtDep_t depv[]);
__global__ void gpuBfs(uint32_t paramc, uint64_t *paramv, uint32_t depc,
                       artsEdtDep_t depv[]);
void cpuBfs(uint32_t paramc, uint64_t *paramv, uint32_t depc,
            artsEdtDep_t depv[]);
void launchSort(uint32_t paramc, uint64_t *paramv, uint32_t depc,
                artsEdtDep_t depv[]);
void cpuSort(uint32_t paramc, uint64_t *paramv, uint32_t depc,
             artsEdtDep_t depv[]);
...
void createFirstRound(uint32_t paramc, uint64_t *paramv, uint32_t depc,
  ...
  if (firstSearchFrontier[0] > GPU_THRESHOLD) {
    dim3 threads(1, 1, 1);
    dim3 grid(1, 1, 1);
    bfsGuid = artsEdtCreateGpu(gpuBfs, artsGetCurrentNode(), 1, &nextLevel, 4,
  ...
  // artsSignalEdt(bfsGuid, 1,
  // nextSearchFrontierAddrGuid[artsGetCurrentNode()]);
...
void cpuBfs(uint32_t paramc, uint64_t *paramv, uint32_t depc,
  ...
  for (unsigned int index = 0; index < currentFrontierSize; index++) {
    ...
    bool success = false;
    while (localLevel < oldLevel) {
      success = (artsAtomicCswap(&localVisited[vertexIndex], oldLevel,
                                 localLevel) == oldLevel);
      oldLevel = localVisited[vertexIndex];
    }
    ...
    if (success) {
      ...
      if (neighborCount) {
        unsigned int frontierIndex =
            artsAtomicFetchAdd(localFrontierCount, (unsigned int)neighborCount);
        if (frontierIndex < GPULISTLEN) {
          for (uint64_t i = 0; i < neighborCount; ++i) {
            local[frontierIndex + i] = neighbors[i];
          }
...
void launchSort(uint32_t paramc, uint64_t *paramv, uint32_t depc,
  ...
  for (unsigned int j = 0; j < artsGetTotalNodes(); j++) {
    for (uint64_t i = 0; i < artsGetTotalGpus(); i++) {
      artsGuid_t thrustGuid = artsEdtCreateGpuLibDirect(thrustSort, j, i, 2,
                                                        args, 0, grid, threads);
    }
...
void cpuSort(uint32_t paramc, uint64_t *paramv, uint32_t depc,
  ...
  unsigned int *local =
      getLocalBuffer(artsGetTotalGpus(),
    ...
    for (unsigned int i = 0; i < PARTS; i++) {
      if (sizePerBound[i]) {
        ...
        *newSearchFrontier = sizePerBound[i];
        ...
        if (!DO_SYNC(localLevel)) {
          artsSignalEdt(edtGuidsToLaunchBfs[i], 1,
                        getBufferGuid(rank, nextLevel));
        }
  ...
  artsSignalEdt(nextLaunchBfsGuid,
                artsGetCurrentNode() * (artsGetTotalGpus() + 1) +
                    artsGetTotalGpus(),
                edtGuidsToLaunchBfsGuid);
}
...
void thrustSort(uint32_t paramc, uint64_t *paramv, uint32_t depc,
  ...
  uint64_t localLevel =
      paramv[0]; // This can be the end if the frontier is empty
  unsigned int *rawPtr = getLocalBuffer(
      artsGetGpuId(), localLevel); // We need the one corresponding to our gpu

  // PRINTF("%s Level: %lu Gpu: %d\n", __func__, localLevel, artsGetGpuId());
  artsGuid_t nextLaunchBfsGuid = paramv[1]; // This is the next sync point.
  artsGuid_t edtGuidsToLaunchBfsGuid =
      NULL_GUID; // Where we will put a copy of all the new edts to start...
  // unsigned int * rawPtr = devPtrRaw[artsGetGpuId()]; //The corresponding dev
...
void launchBfs(uint32_t paramc, uint64_t *paramv, uint32_t depc,
  ...
  if (localLevel < MAXLEVEL) {
    ...
    unsigned int numPotentialBfsDbs =
        artsGetTotalNodes() * (artsGetTotalGpus() + 1);
        ...
        for (unsigned int j = 0; j < PARTS; j++) {
          if (guidToSignal[j]) {
            if (DO_SYNC(lastLevel)) {
              ...
              artsSignalEdt(guidToSignal[j], 1,
                            getBufferGuid(rank, localLevel));
            }
            totalNewBfs++;
          }
        }
