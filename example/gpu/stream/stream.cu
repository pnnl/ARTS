...
__global__ void copyKernel(uint32_t paramc, uint64_t *paramv, uint32_t depc,
                           artsEdtDep_t depv[]) {
...
__global__ void scaleKernel(uint32_t paramc, uint64_t *paramv, uint32_t depc,
                            artsEdtDep_t depv[]) {
  unsigned int len = (unsigned int)paramv[0];
...
__global__ void addKernel(uint32_t paramc, uint64_t *paramv, uint32_t depc,
                          artsEdtDep_t depv[]) {
  unsigned int len = (unsigned int)paramv[0];
  double *a = (double *)depv[0].ptr;
  double *b = (double *)depv[1].ptr;
...
__global__ void triadKernal(uint32_t paramc, uint64_t *paramv, uint32_t depc,
                            artsEdtDep_t depv[]) {
  unsigned int len = (unsigned int)paramv[0];
  double scale = (double)paramv[1];
  double *a = (double *)depv[0].ptr;
...
extern "C" void initPerNode(unsigned int nodeId, int argc, char **argv) {
  ...
  aTileGuids = artsNewGuidRangeNodeHash(ARTS_DB_GPU_WRITE, numTiles, 0,
                                        artsGetTotalGpus());
  bTileGuids = artsNewGuidRangeNodeHash(ARTS_DB_GPU_WRITE, numTiles, 0,
                                        artsGetTotalGpus());
  cTileGuids = artsNewGuidRangeNodeHash(ARTS_DB_GPU_WRITE, numTiles, 0,
                                        artsGetTotalGpus());
...
#ifdef SAFE
  if (artsGetNumGpus() > 1) {
    if (!(artsLookUpConfig(freeDbAfterGpuRun) &&
          artsLookUpConfig(runGpuGcPreEdt))) {
      if (artsLookUpConfig(gpuLocality) != 3 || aHash != bHash ||
          aHash != cHash) {
        PRINTF("For more than 1 GPU Stream requires gpuLocality to be set to "
               "3.\n");
