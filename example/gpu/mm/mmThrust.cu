...
void multiplyMM(uint32_t paramc, uint64_t *paramv, uint32_t depc,
  ...
  artsGuid_t cTileGuid =
      artsDbCreate((void **)&cTileHost, size, ARTS_DB_GPU_WRITE);
...
void sumMM(uint32_t paramc, uint64_t *paramv, uint32_t depc,
           artsEdtDep_t depv[]) {
  ...
  double *cTile = NULL;
  artsGuid_t cTileGuid = artsDbCreate(
      (void **)&cTile, sizeof(double) * tileSize * tileSize, ARTS_DB_GPU_WRITE);
  ...
  for (unsigned int k = 1; k < depc; ++k) {
    ...
    thrust::device_vector<double> toAddTileDev(
        toAddPtrDev, toAddPtrDev + (tileSize * tileSize));
    thrust::transform(cTileDev.begin(), cTileDev.end(), toAddTileDev.begin(),
                      cTileDev.begin(), thrust::plus<double>());
  }

  artsPutInDbFromGpu(thrust::raw_pointer_cast(cTileDev.data()), cTileGuid, 0,
                     sizeof(double) * tileSize * tileSize, false);
  ...
  for (unsigned int i = 0; i < numBlocks; i++) {
    for (unsigned int j = 0; j < numBlocks; j++) {
      if ((i * numBlocks + j) % totalThreads == globalThreadId) {
        ...
        artsGuid_t sumGuid = artsEdtCreateGpuLib(sumMM, nodeId, 3, sumArgs,
                                                 numBlocks, grid, threads);
        for (unsigned int k = 0; k < numBlocks; k++) {
          uint64_t args[] = {(uint64_t)sumGuid, i, j, k};
          artsGuid_t mulGuid = artsEdtCreateGpuLib(multiplyMM, nodeId, 4, args,
                                                   2, grid, threads);
