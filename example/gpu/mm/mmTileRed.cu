...
unsigned int reserveEdtGuids(artsGuid_t *allGuids, unsigned int index,
                             artsType_t edtType) {
  if (allGuids[index])
...
binaryReductionTree_t *
initBinaryReductionTree(unsigned int numLeaves, artsEdt_t funPtr,
                        artsType_t dbType, artsType_t edtType, uint32_t paramc,
                        uint64_t *paramv, dim3 grid, dim3 block,
  ...
  for (unsigned int i = 0; i < tree->interiorNodes; i++) {
    if (artsIsGuidLocal(tree->redEdtGuids[i])) {
      if (!i) {
        PRINTF("Last: %lu -> %lu slot: %u\n", tree->redEdtGuids[i], endGuid,
               slot);
        artsEdtCreateGpuPTWithGuid(funPtr, tree->redEdtGuids[i], paramc, paramv,
                                   2, grid, block, endGuid, slot, 0);
      } else {
        ...
        PRINTF("%lu -> %lu slot: %u parent: %d\n", tree->redEdtGuids[i],
               toSignal, toSignalSlot, parentIndex);
        artsEdtCreateGpuPTWithGuid(funPtr, tree->redEdtGuids[i], paramc, paramv,
                                   2, grid, block, toSignal, toSignalSlot, 0);
      }
    }
  }
...
void multiplyMM(uint32_t paramc, uint64_t *paramv, uint32_t depc,
                artsEdtDep_t depv[]) {
  ...
  cublasDgemm(handle[artsGetGpuId()], CUBLAS_OP_N, CUBLAS_OP_N, tileSize,
              tileSize, tileSize, &alpha, aTileDev, tileSize, bTileDev,
              tileSize, &beta, cTileDev, tileSize);
...
__global__ void sumMMKernel(uint32_t paramc, uint64_t *paramv, uint32_t depc,
                            artsEdtDep_t depv[]) {
...
void finishBlockMM(uint32_t paramc, uint64_t *paramv, uint32_t depc,
                   artsEdtDep_t depv[]) {
...
extern "C" void initPerNode(unsigned int nodeId, int argc, char **argv) {
  ...
  if (!nodeId) {
    aMatrix = (double *)artsDbCreateWithGuid(aMatGuid, matSize * matSize *
                                                           sizeof(double));
    bMatrix = (double *)artsDbCreateWithGuid(bMatGuid, matSize * matSize *
                                                           sizeof(double));
    cMatrix = (double *)artsDbCreateWithGuid(cMatGuid, matSize * matSize *
                                                           sizeof(double));
...
extern "C" void initPerWorker(unsigned int nodeId, unsigned int workerId,
  ...
  if (!nodeId && !workerId) {
    for (unsigned int i = 0; i < numBlocks; i++) {
      for (unsigned int j = 0; j < numBlocks; j++) {
        ...
        double *aTile = (double *)artsDbCreateWithGuid(
            aTileGuid, sizeof(double) * tileSize * tileSize);
        copyBlock(i, j, tileSize, aTile, matSize, aMatrix, true);

        artsGuid_t bTileGuid = artsGetGuid(bTileGuids, i * numBlocks + j);
        double *bTile = (double *)artsDbCreateWithGuid(
            bTileGuid, sizeof(double) * tileSize * tileSize);
  ...
  for (unsigned int i = 0; i < numBlocks; i++) {
    for (unsigned int j = 0; j < numBlocks; j++) {
      if ((i * numBlocks + j) % totalThreads == globalThreadId) {
        // artsGuid_t sumGuid = artsEdtCreateGpuPT (sumMMKernel, nodeId, 1,
        // sumArgs, numBlocks, grid, threads, doneGuid, 3 + (i * numBlocks + j),
        // 0);
        ...
        for (unsigned int k = 0; k < numBlocks; k++) {
          ...
          artsGuid_t mulGuid = artsEdtCreateGpuLib(multiplyMM, nodeId, 5, args,
                                                   2, grid, threads);
  ...
  if (!nodeId && !workerId) {
    artsEdtCreateWithGuid(finishBlockMM, doneGuid, 0, NULL,
                          3 + numBlocks * numBlocks);
...
extern "C" void initPerGpu(unsigned int nodeId, int devId, cudaStream_t *stream,
  ...
  if (!devId)
    handle =
        (cublasHandle_t *)artsCalloc(artsGetNumGpus(), sizeof(cublasHandle_t));
