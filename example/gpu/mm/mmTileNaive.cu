...
__global__ void mmKernel(uint32_t paramc, uint64_t *paramv, uint32_t depc,
                         artsEdtDep_t depv[]) {
...
void mmKernelCPU(uint32_t paramc, uint64_t *paramv, uint32_t depc,
                 artsEdtDep_t depv[]) {
...
void multiplyMM(uint32_t paramc, uint64_t *paramv, uint32_t depc,
  ...
  artsGuid_t aTileGuid =
      artsDbCreate((void **)&aTile, sizeof(double) * tile_size * tile_size,
                   ARTS_DB_GPU_READ);
  artsGuid_t bTileGuid =
      artsDbCreate((void **)&bTile, sizeof(double) * tile_size * tile_size,
                   ARTS_DB_GPU_READ);
  artsGuid_t cTileGuid =
      artsDbCreate((void **)&cTile, sizeof(double) * tile_size * tile_size,
                   ARTS_DB_GPU_WRITE);
  ...
  artsGuid_t mulGpuGuid =
      artsEdtCreateGpu(mmKernel, artsGetCurrentNode(), 1, args, 3, grid,
                       threads, toSignal, k, cTileGuid);
  ...
  artsGuid_t mulGpuGuid =
      artsEdtCreate(mmKernelCPU, artsGetCurrentNode(), 4, args, 3);
  artsSignalEdt(mulGpuGuid, 0, aTileGuid);
  artsSignalEdt(mulGpuGuid, 1, bTileGuid);
  artsSignalEdt(mulGpuGuid, 2, cTileGuid);
#endif
  ...
  for (unsigned int i = 0; i < mat_size; ++i)
    for (unsigned int j = 0; j < mat_size; ++j)
      for (unsigned int k = 0; k < mat_size; ++k)
        temp[i * mat_size + j] +=
            aMat[i * mat_size + k] * bMat[k * mat_size + j];
...
extern "C" void initPerNode(unsigned int nodeId, int argc, char **argv) {
  ...
  if (!nodeId) {
    double *aMat = (double *)artsDbCreateWithGuid(
        aMatGuid, mat_size * mat_size * sizeof(double));
    double *bMat = (double *)artsDbCreateWithGuid(
        bMatGuid, mat_size * mat_size * sizeof(double));
    double *cMat = (double *)artsDbCreateWithGuid(
        cMatGuid, mat_size * mat_size * sizeof(double));
    ...
    for (unsigned int j = 0; j < numBlocks; j++)
      if ((i * numBlocks + j) % totalThreads == globalThreadId) {
        ...
        dim3 grid((tile_size + SMTILE - 1) / SMTILE,
                  (tile_size + SMTILE - 1) / SMTILE);
        artsGuid_t sumGuid =
            artsEdtCreateGpuPT(sumMMKernel, nodeId, 1, sumArgs, numBlocks, grid,
                               threads, doneGuid, 3 + (i * numBlocks + j), 0);
#else
        uint64_t sumArgs[] = {doneGuid, i, j};
        artsGuid_t sumGuid =
            artsEdtCreate(sumMM, nodeId, 3, sumArgs, numBlocks);
#endif
